...
/* =========================================================
   FOR REVIEW — REVIEW CODE003 / [รีวิวภาพยนตร์/ซีรีส์] blue hour
...
(() => {
  ...
  const ids = Object.freeze({
    ...
    heroX: "movieReviewHeroX",
    heroY: "movieReviewHeroY",
  ...
  function buildMarkup(values, previewMode = false) {
    ...
    const hasHeroPosition = String(values.heroX ?? "").trim() !== "" || String(values.heroY ?? "").trim() !== "";
    const heroX = clamp(values.heroX, 0, 100, 50);
    const heroY = clamp(values.heroY, 0, 100, 50);
    const heroPositionStyle = hasHeroPosition ? ` style="background-position:${heroX}% ${heroY}%;"` : "";
  ...
  function buildNamedUi(host, panel) {
    ...
    wrap.innerHTML = `
      ...
      <div class="dds-named-save-heading">
        <strong>ไฟล์ที่บันทึกไว้</strong>
      </div>
...
(() => {
  ...
  const previewStylesheetUrl = stylesheetUrl;
  ...
  function colorToRgbTuple(raw, fallback = "201, 182, 174") {
    ...
    const rgbMatch = /^rgba?\((\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})/i.exec(value);
...
.${scope} .ddsh-chocolove-photo-image{background-position:${photoX}% ${photoY}%!important;background-size:${photoSize}!important;}
  ...
  function fitCardPreviewImmediately(iframe) {
    if (!iframe) return;
    const stage = iframe.closest(".dds-roleplay-card-preview");
    const iframeWidth = 682;
    const iframeHeight = 860;
    const availableWidth = Math.max(1, (stage?.clientWidth || 420) - 28);
    const availableHeight = Math.max(1, (stage?.clientHeight || 260) - 28);
    const scale = Math.max(0.01, Math.min(1, availableWidth / iframeWidth, availableHeight / iframeHeight));
    iframe.style.width = `${iframeWidth}px`;
    iframe.style.height = `${iframeHeight}px`;
    iframe.style.left = "50%";
  ...
  function renderPreview(iframe, markup, isCard) {
    ...
    if (isCard) {
      fitCardPreviewImmediately(iframe);
      if (state.srcdoc === srcdoc && iframe.srcdoc === srcdoc) return;
      state.srcdoc = srcdoc;
      state.loaded = false;
      previewStates.set(iframe, state);
      iframe.addEventListener("load", () => {
      ...
      iframe.srcdoc = srcdoc;
      requestAnimationFrame(() => fitCardPreviewImmediately(iframe));
      return;
    }
    state.token += 1;
    const token = state.token;
  ...
  function initialize() {
    ...
    function readValues() {
      return {
        ...
        photoZoom: Number(value("chocolovePhotoZoom", 100)),
...
(() => {
  ...
  function buildCode(values = defaults, previewMode = false) {
    ...
    const photoBorderColor = normalizeColor(v.photoBorderColor, defaults.photoBorderColor);
    const noteBorderColor = normalizeColor(v.noteBorderColor, defaults.noteBorderColor);
    const noteTextColor = normalizeColor(v.noteTextColor, defaults.noteTextColor);
      ...
      <div class="dds-protected-commission-layout">
        ...
        <div class="dds-protected-commission-controls-column">
          ...
          <div class="dds-protected-commission-scroll dds-jewel-commission-scroll">
            ...
            <section class="dds-control-section"><div class="dds-control-title"><span>03</span><h2>สีของโคด</h2></div><div class="dds-color-grid">${colorField("สีกรอบโคด", "frameColor")}${colorField("สีขอบรูปในโคด", "photoBorderColor")}${colorField("สีกรอบหมายเหตุ", "noteBorderColor")}${colorField("สีข้อความในหมายเหตุ", "noteTextColor")}${colorField("สีขีด", "lineColor")}${colorField("สี Symbol", "symbolColor")}${colorField("สีตัวอักษรโควต", "quoteColor")}${colorField("สีเนื้อหาโรลเพลย์", "roleplayColor")}</div></section>
...
(() => {
  ...
  const DRAFT_KEY = "dds:commission-draft:landon-file-roleplay:v2";
  const ACCESS_HASH = "6d05de9e9a208dc2beb7d5e594b39064b36142353ffc8db10295131098a1bcd6";
      ...
      <div class="dds-protected-commission-layout">
        ...
        <div class="dds-protected-commission-controls-column">
          ...
          <div class="dds-protected-commission-scroll dds-landon-file-commission-scroll">
            ...
            <section class="dds-control-section"><div class="dds-control-title"><span>02</span><h2>รูปภาพ</h2></div><div class="dds-form-grid"><label class="dds-field dds-field-full"><span>รูปโพลารอยด์</span><input type="url" data-landon-file-field="photoImage" spellcheck="false"></label><div class="dds-image-position dds-field-full"><div class="dds-image-position-heading"><span>ตำแหน่งและซูมรูปโพลารอยด์</span><small>ปรับซ้าย–ขวา และซูมเข้า–ออก</small></div><label class="dds-position-row"><span>แนวนอน</span><small>ซ้าย</small><input type="range" min="0" max="100" step="1" data-landon-file-field="photoX"><small>ขวา</small><output data-landon-file-output="photoX">50%</output></label><label class="dds-position-row dds-zoom-position-row"><span>ซูมรูป</span><small>ออก</small><input type="range" min="50" max="200" step="1" data-landon-file-field="photoZoom"><small>เข้า</small><output data-landon-file-output="photoZoom">100%</output></label></div><label class="dds-field"><span>ข้อความใต้โพลารอยด์ — ซ้าย</span><input type="text" data-landon-file-field="photoFileName" placeholder="IMG_0525.JPG"></label><label class="dds-field"><span>ข้อความใต้โพลารอยด์ — ขวา</span><input type="text" data-landon-file-field="photoTime" placeholder="01:17 AM"></label><label class="dds-field dds-field-full"><span>รูปด้านหลัง</span><input type="url" data-landon-file-field="backImage" spellcheck="false"></label><div class="dds-image-position dds-field-full"><div class="dds-image-position-heading"><span>ตำแหน่งและซูมรูปด้านหลัง</span><small>ปรับซ้าย–ขวา และซูมเข้า–ออก</small></div><label class="dds-position-row"><span>แนวนอน</span><small>ซ้าย</small><input type="range" min="0" max="100" step="1" data-landon-file-field="backX"><small>ขวา</small><output data-landon-file-output="backX">50%</output></label><label class="dds-position-row dds-zoom-position-row"><span>ซูมรูป</span><small>ออก</small><input type="range" min="50" max="200" step="1" data-landon-file-field="backZoom"><small>เข้า</small><output data-landon-file-output="backZoom">100%</output></label></div></div></section>
            <section class="dds-control-section"><div class="dds-control-title"><span>03</span><h2>ข้อมูลส่วนหัว</h2></div><div class="dds-form-grid"><label class="dds-field dds-field-full"><span>Locations / สถานที่</span><input type="text" data-landon-file-field="location" placeholder="กรอกสถานที่"></label><label class="dds-field dds-field-full"><span>QUOTE</span><input type="text" data-landon-file-field="quote" placeholder="กรอก QUOTE"></label><label class="dds-field"><span>ชื่อบรรทัดบน</span><input type="text" data-landon-file-field="firstName"></label><label class="dds-field"><span>นามสกุลบรรทัดล่าง</span><input type="text" data-landon-file-field="lastName"></label><label class="dds-field"><span>FILE</span><input type="text" data-landon-file-field="fileLabel"></label><label class="dds-field"><span>วันที่</span><input type="text" data-landon-file-field="dateLabel"></label></div></section>
...
(() => {
  ...
  function bindPanel() {
    ...
    panel.querySelector("[data-code013-reset]")?.addEventListener("click", () => { setValues(editorDefaults); updatePreview(); scheduleDraftSave(); notify("รีเซ็ต CODE013 แล้ว"); });
  }
...
(() => {
  ...
  const defaults = Object.freeze({
    ...
    cassette:"https://iili.io/nnRQmnj.png", cassetteLink:"LINK-CASSETTE", cassetteX:0, cassetteY:0,
...
.ddsh-commitalan-ref-side-bottom{translate:${+v.sideBottomX}px ${+v.sideBottomY}px !important}
    ...
    return `<link href="${CSS_URL}" rel="stylesheet"><link href="${FONT_URL}" rel="stylesheet"><div class="ddsh-commitalan-ref" style="${styleVars};"><div class="ddsh-commitalan-ref-main"><div class="ddsh-commitalan-ref-photo ddsh-commitalan-ref-photo1"><img src="${attr(v.photo1)}" alt="" style="object-position:${+v.photo1x}% ${+v.photo1y}% !important;scale:${(+v.photo1zoom/100).toFixed(3)} !important;"></div><div class="ddsh-commitalan-ref-photo ddsh-commitalan-ref-photo2"><img src="${attr(v.photo2)}" alt="" style="object-position:${+v.photo2x}% ${+v.photo2y}% !important;scale:${(+v.photo2zoom/100).toFixed(3)} !important;"></div><div class="ddsh-commitalan-ref-photo ddsh-commitalan-ref-photo3"><img src="${attr(v.photo3)}" alt="" style="object-position:${+v.photo3x}% ${+v.photo3y}% !important;scale:${(+v.photo3zoom/100).toFixed(3)} !important;"></div><div class="ddsh-commitalan-ref-photo ddsh-commitalan-ref-photo4"><img src="${attr(v.photo4)}" alt="" style="object-position:${+v.photo4x}% ${+v.photo4y}% !important;scale:${(+v.photo4zoom/100).toFixed(3)} !important;"></div><div class="ddsh-commitalan-ref-grain"></div><div class="ddsh-commitalan-ref-scratch"></div><div class="ddsh-commitalan-ref-top" style="translate:${+v.topX}px ${+v.topY}px !important;"><div class="ddsh-commitalan-ref-top-main">${esc(v.topMain)}</div><div class="ddsh-commitalan-ref-top-script">${esc(v.topScript)}</div></div><div class="ddsh-commitalan-ref-title"><span style="position:relative !important;left:${+v.title1x}px !important;top:${+v.title1y}px !important;">${esc(v.title1)}</span><span style="position:relative !important;left:${+v.title2x}px !important;top:${+v.title2y}px !important;">${esc(v.title2)}</span></div><a href="${attr(v.note1Link)}" target="_blank" class="ddsh-commitalan-ref-note ddsh-commitalan-ref-note1" style="translate:${+v.note1x}px ${+v.note1y}px !important;">${esc(v.note1Text)}</a><a href="${attr(v.note2Link)}" target="_blank" class="ddsh-commitalan-ref-note ddsh-commitalan-ref-note2" style="translate:${+v.note2x}px ${+v.note2y}px !important;">${esc(v.note2Text)}</a><a href="${attr(v.note3Link)}" target="_blank" class="ddsh-commitalan-ref-note ddsh-commitalan-ref-note3" style="translate:${+v.note3x}px ${+v.note3y}px !important;">${esc(v.note3Text)}</a><a href="${attr(v.note4Link)}" target="_blank" class="ddsh-commitalan-ref-note ddsh-commitalan-ref-note4" style="translate:${+v.note4x}px ${+v.note4y}px !important;">${esc(v.note4Text)}</a><a href="${attr(v.note5Link)}" target="_blank" class="ddsh-commitalan-ref-note ddsh-commitalan-ref-note5" style="translate:${+v.note5x}px ${+v.note5y}px !important;">${esc(v.note5Text)}</a><div class="ddsh-commitalan-ref-star ddsh-commitalan-ref-star1" style="translate:${+v.star1x}px ${+v.star1y}px !important;">${esc(v.star1)}</div><div class="ddsh-commitalan-ref-star ddsh-commitalan-ref-star2" style="translate:${+v.star2x}px ${+v.star2y}px !important;">${esc(v.star2)}</div><a href="${attr(v.cassetteLink)}" target="_blank" rel="noopener noreferrer" class="ddsh-commitalan-ref-cassette" style="translate:${+v.cassetteX}px ${+v.cassetteY}px !important;"><img src="${attr(v.cassette)}" alt=""></a><div class="ddsh-commitalan-ref-rip"></div></div><aside class="ddsh-commitalan-ref-side"><div class="ddsh-commitalan-ref-side-script" style="translate:${+v.sideScriptX}px ${+v.sideScriptY}px !important;">${esc(v.sideScript)}</div><div class="ddsh-commitalan-ref-side-copy" style="translate:${+v.sideCopyX}px ${+v.sideCopyY}px !important;">${esc(v.sideCopy)}</div><div class="ddsh-commitalan-ref-side-bottom" style="translate:${+v.sideBottomX}px ${+v.sideBottomY}px !important;">${esc(v.sideBottom)}</div></aside></div><div class="ddshopfz-ccm01"><span></span></div>`;
  }
        ...
        <div class="dds-protected-commission-controls-column">
          ...
          <div class="dds-alan-controls-scroll">
            ...
            <section class="dds-control-section"><div class="dds-control-title"><span>14</span><h2>เทปคลาสเซ็ท</h2></div><div class="dds-form-grid">
              ${textField("URL รูปเทป","cassette",defaults.cassette,true)}${textField("ลิงก์เมื่อกดเทป","cassetteLink",defaults.cassetteLink,true)}${xyControls("cassette",0,0)}
            </div></section>
...
/* =========================================================
   DDS CLOUD SAVE — IMPORT DEVICE + CLOUD-FIRST v113
    ...
    modal.innerHTML = `
      <div class="dds-account-cloud-dialog">
        ...
        <section class="dds-account-cloud-info">
          ...
          <p>Login แล้วเซฟของทุก Editor จะใช้ Cloud ของบัญชีนี้อัตโนมัติ</p>
        </section>
  ...
  async function initAuth() {
    ...
    try {
      ...
      if (sessionUser && activeId === sessionUser.id) {
        ...
        await refreshCloudIntoAccount({ reloadIfChanged: false });
      }
...
/* =========================================================
   MIKAEL F. KAISER — FREE HISTORY COMMISSION v125
   โคดประเภทประวัติ (ทุกคนใช้ได้ฟรี)
   Public preview / View Work enabled / Editor open access
========================================================= */
(() => {
  "use strict";

  if (window.__DDS_MIKAEL_HISTORY_FREE_V125__) return;
  window.__DDS_MIKAEL_HISTORY_FREE_V125__ = true;

  const PANEL_NAME = "editor-commission-mikael-history-free";
  const VIEW_PANEL_NAME = "view-commission-mikael-history-free";
  ...
  const OFFICIAL_CODE = "<style>.ddsh-commtmklpfofile .ddsh-commtmklpfo-barcode{background:repeating-linear-gradient(90deg,var(--ddsh-commtmklpfo-ink) 0 2px,transparent 2px 4px,var(--ddsh-commtmklpfo-ink) 4px 5px,transparent 5px 8px,var(--ddsh-commtmklpfo-ink) 8px 11px,transparent 11px 13px,var(--ddsh-commtmklpfo-ink) 13px 14px,transparent 14px 18px)!important;border-color:var(--ddsh-commtmklpfo-ink)!important;box-shadow:none!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-barcode::before,.ddsh-commtmklpfofile .ddsh-commtmklpfo-barcode::after{background:var(--ddsh-commtmklpfo-ink)!important;border-color:var(--ddsh-commtmklpfo-ink)!important;color:var(--ddsh-commtmklpfo-ink)!important}.ddsh-commtmklpfofile .ddsh-blackline,.ddsh-commtmklpfofile .ddsh-mini-redact,.ddsh-commtmklpfofile .ddsh-redact,.ddsh-commtmklpfofile .ddsh-commtmklpfo-inline-redaction span,.ddsh-commtmklpfofile .ddsh-commtmklpfo-classified-note b,.ddsh-commtmklpfofile .ddsh-commtmklpfo-bottomredact i{background:var(--ddsh-commtmklpfo-ink)!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-redstamp,.ddsh-commtmklpfofile .ddsh-commtmklpfo-footerstamp{color:var(--ddsh-commtmklpfo-stamp)!important;border-color:var(--ddsh-commtmklpfo-stamp)!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-photo::before{transform:translate(var(--ddsh-commtmklpfo-photo-x,0px),var(--ddsh-commtmklpfo-photo-y,0px)) scale(var(--ddsh-commtmklpfo-photo-zoom,1))!important;transform-origin:center center!important}</style><link href=\"https://guindaeyo.github.io/commisdeepdcsh/comm-mmklpfofile.css\" rel=\"stylesheet\"><link href=\"https://fonts.googleapis.com/css2?family=Bai+Jamjuree:wght@400;500;600;700&display=swap\" rel=\"stylesheet\"><div class=\"ddsh-commtmklpfofile\" style=\"--ddsh-commtmklpfo-layout-width:1280px;--ddsh-commtmklpfo-paper:#e7e5df;--ddsh-commtmklpfo-ink:#000000;--ddsh-commtmklpfo-muted:rgba(0,0,0,0.68);--ddsh-commtmklpfo-line:rgba(0,0,0,0.74);--ddsh-commtmklpfo-light:rgba(0,0,0,0.32);--ddsh-commtmklpfo-red:#b00000;--ddsh-commtmklpfo-stamp:#b00000;--ddsh-commtmklpfo-logo:url('https://iili.io/nxjgK22.png');--ddsh-commtmklpfo-photo:url('https://i.pinimg.com/736x/ed/87/f8/ed87f8e32c188c1626741d0c0baab337.jpg');--ddsh-commtmklpfo-photo-x:0px;--ddsh-commtmklpfo-photo-y:5px;--ddsh-commtmklpfo-photo-zoom:1.000;--ddsh-commtmklpfo-redstamp-x:0px;--ddsh-commtmklpfo-redstamp-y:0px\"><div class=\"ddsh-commtmklpfo-paperfx\"></div><div class=\"ddsh-commtmklpfo-content\"><header class=\"ddsh-commtmklpfo-header\"><div class=\"ddsh-commtmklpfo-dept\"><div class=\"ddsh-commtmklpfo-police-logo\"></div><div class=\"ddsh-commtmklpfo-depttext\"><span>ELYSIAN POLICE DEPARTMENT</span><strong>RECORDS & INVESTIGATION DIVISION</strong><small>CONFIDENTIAL PERSONNEL ARCHIVE</small></div></div><div class=\"ddsh-commtmklpfo-title\"><h1>CASE FILE</h1><span>OFFICIAL PERSONNEL RECORD</span></div><div class=\"ddsh-commtmklpfo-number\"><div class=\"ddsh-commtmklpfo-numbertop\"><span>CASE NO.</span><strong>EPD-22-3345</strong></div><div class=\"ddsh-commtmklpfo-barcode\"></div><div class=\"ddsh-commtmklpfo-barcode-number\">8 920176 002344</div><small>RESTRICTED DATABASE RECORD</small></div></header><div class=\"ddsh-commtmklpfo-heavyline\"></div><section class=\"ddsh-commtmklpfo-main\"><div class=\"ddsh-commtmklpfo-photos\"><div class=\"ddsh-commtmklpfo-photo\"><div class=\"ddsh-commtmklpfo-height\"><span>190</span><span>180</span><span>170</span><span>160</span><span>150</span></div><div class=\"ddsh-commtmklpfo-photo-id\"><strong>EPD</strong><span>SUBJECT VIEW</span><b>0023-44</b></div></div></div><div class=\"ddsh-commtmklpfo-identification\"><div class=\"ddsh-commtmklpfo-blocktitle\"><span>SUBJECT IDENTIFICATION</span><b>FILE: ACTIVE</b></div><div class=\"ddsh-commtmklpfo-name\"><span>FULL NAME (THAI)</span><strong>มิคาเอล เฟอร์รอส ไคเซอร์</strong><small>LEGAL IDENTITY CONFIRMED</small></div><div class=\"ddsh-commtmklpfo-info\"><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">ALIASES</span><span class=\"ddsh-commtmklpfo-value\">Mikael, Kel, Kelly</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">FACECLAIM</span><span class=\"ddsh-commtmklpfo-value\">Huening Kai</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">BIRTHPLACE</span><span class=\"ddsh-commtmklpfo-value\">London, UK</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">DATE OF BIRTH</span><span class=\"ddsh-commtmklpfo-value\">29/09/1859</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">SPECIES</span><span class=\"ddsh-commtmklpfo-value\">Vampire</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">LEVEL</span><span class=\"ddsh-commtmklpfo-value\">4</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">AGE</span><span class=\"ddsh-commtmklpfo-value\">165 YEARS</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">MBTI</span><span class=\"ddsh-commtmklpfo-value\">INFJ</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">EDUCATION</span><span class=\"ddsh-commtmklpfo-value\">Master of Economics</span></div><div class=\"ddsh-commtmklpfo-row\"><span class=\"ddsh-commtmklpfo-label\">OCCUPATION</span><span class=\"ddsh-commtmklpfo-value\">Producer, Pianist</span></div></div><div class=\"ddsh-commtmklpfo-subject-note\"><div class=\"ddsh-commtmklpfo-subject-note-head\"><span>NOTE</span><b>SUBJECT REMARK</b></div><p>Married</p></div></div><aside class=\"ddsh-commtmklpfo-side\"><div class=\"ddsh-commtmklpfo-sidebox\"><span>CASE STATUS</span><strong>ACTIVE FILE</strong><small>RECORD UNDER REVIEW</small></div><div class=\"ddsh-commtmklpfo-sidebox\"><span>SECURITY LEVEL</span><strong>CLASSIFIED</strong><small>AUTHORIZED PERSONNEL ONLY</small></div><div class=\"ddsh-commtmklpfo-sidebox\"><span>DATABASE STATUS</span><strong>VERIFIED</strong><small>LAST UPDATE: 05/29/2026</small></div><div class=\"ddsh-commtmklpfo-sidebox\"><span>REFERENCE</span><strong>EPD-2203</strong><small><span class=\"ddsh-redact ddsh-redact-long\">REDACTED DATA</span></small></div><div class=\"ddsh-commtmklpfo-redstamp\"><strong>ARCHIVED</strong><span>PERSONAL RECORD</span></div></aside></section><div class=\"ddsh-commtmklpfo-photo-caption\"><span>SUBJECT IMAGE / IDENTIFICATION RECORD NO. EPD-0023-44</span><span>IMAGE SOURCE: <b class=\"ddsh-mini-redact\"></b></span></div><section class=\"ddsh-commtmklpfo-chargebox\"><div class=\"ddsh-commtmklpfo-charge-title\">FILE INFORMATION</div><div class=\"ddsh-commtmklpfo-charge-grid\"><div><span>RECORD TYPE</span><strong>PERSONAL HISTORY</strong></div><div><span>DOCUMENT STATUS</span><strong>COMPLETE RECORD</strong></div><div><span>ACCESS LEVEL</span><strong>CONFIDENTIAL</strong></div><div><span>FILE OWNER</span><strong class=\"ddsh-blackline\"></strong></div></div></section><section class=\"ddsh-commtmklpfo-section\"><div class=\"ddsh-commtmklpfo-sectionhead\"><strong>01</strong><div><span>BACKGROUND RECORD</span><h2>HISTORY</h2></div><small>SUBJECT HISTORY</small></div><div class=\"ddsh-commtmklpfo-text\"><div class=\"ddsh-commtmklpfo-recordline\"><span>REPORTING OFFICER:</span><b class=\"ddsh-blackline ddsh-blackline-md\"></b><span>DATE:</span><strong>30/9/2026</strong></div><p>มิคาเอลเป็นลูกชายเพียงคนเดียวของตระกูลขุนนางชั้นเอิร์ลที่มั่งคั่งอย่างไคเซอร์ ด้วยการครอบครองเหมืองแร่ และกิจการเจียระไนเพชรพลอย จึงทำให้ตระกูลเอิร์ลไคเซอร์ เป็นตระกูลที่มั่งคั่งไปด้วยทรัพย์สินเงินทองอย่างมหาศาล มิคาเอลมีรูปร่างสูงโปร่ง หน้าตาที่หล่อเหลา ผิวขาว และดวงตาสีสวยอันเป็นที่ต้องมนต์ทุกคราที่ผู้ใดได้สบตา เขาเป็นชายหนุ่มที่นอกจากรูปงามแล้ว ยังฉลาดหลักแหลมอีกด้วย มิคาเอลเติบโตมาท่ามกลางความรักของบิดาและมารดา เขาที่ใช้ชีวิตมาอย่างดีท่ามกลางครอบครัวที่อบอุ่นและเงินทองอันมหาศาล แต่แล้วในวันหนึ่งที่เขาอายุได้ 25 ปี เขาได้ล้มป่วยกะทันหันด้วยโรคที่ไม่สามารถรักษาได้ เอิร์ลและเคาท์เตสไคเซอร์ได้เชิญหมอฝีมือดีมาทั่วทุกสารทิศ เรียกได้ว่าแทบจะพลิกแผ่นดินหา แม้กระทั่งหมอที่ได้ฉายาว่าเป็นหมอเทวดา ก็ไม่สามารถวินิจฉัยสาเหตุของโรคได้ ชายหนุ่มทำได้เพียงยอมรับชะตากรรมและใช้ชีวิตไปอย่างรอความตาย<br><br>เอิร์ลและเคาท์เตสไคเซอร์นั้นทำใจไม่ได้ที่จะต้องทนเห็นลูกชายหัวแก้วหัวแหวนของตัวเองนั้นต้องตาย วันหนึ่ง เอิร์ลและเคาท์เตสไคเซอร์ไปได้ยินข่าวลือที่ว่ามีคนที่ใกล้ตายด้วยโรคที่ไม่มีทางรักษาหายได้แล้วกลับมาแข็งแรงและใช้ชีวิตได้อย่างปกติ คราแรก พวกเขาไม่ได้เชื่อข่าวลือนั่น แต่ด้วยความที่พวกเขาเองก็อับจนหนทางที่จะรักษาลูกชายของพวกเขาแล้ว จึงได้เดินทางไปพบคนผู้นั้นตามข่าวลือ<br><br>เมื่อเดินทางไปถึง พวกเขาได้พบกับอาร์เธอร์ วินยาร์ด ลูกชายของบารอนวินยาร์ดที่ปกครองอาณาเขตนี้อยู่ อาร์เธอร์เล่าให้สองสามีภรรยาฟังว่าตอนที่เขาใกล้จะสิ้นใจ ท่านบารอนวินยาร์ดที่เป็นพ่อของเขานั้น ได้พาคนแปลก ๆ ผู้หนึ่งมา เขาทำการถ่ายเลือดของเขาให้กับอาร์เธอร์ หลังจากนั้นอาร์เธอร์ก็ได้รอดพ้นจากความตายและกลายเป็นแวมไพร์ในที่สุด นั่นจึงทำให้อาร์เธอร์รู้ว่าพ่อของเขานั้นได้ทำสัญญาลับ ๆ บางอย่างกับแวมไพร์ตนนั้น เอิร์ลและเคาท์เตสไคเซอร์เมื่อได้ยินดังนั้น ก็เกิดการลังเลขึ้นในใจลึก ๆ ว่าจะให้มิคาเอลนั้นกลายเป็นแวมไพร์ หรือปล่อยให้เขาจากไปอย่างสงบ แต่ด้วยความที่ทั้งคู่ไม่อยากให้ลูกชายสุดที่รักที่ใกล้สิ้นใจเต็มทีนั้นต้องตายในวัยหนุ่ม พวกเขาจึงได้ยื่นข้อเสนอให้อาร์เธอร์นั้น ทำการเปลี่ยนลูกชายของตนให้กลายเป็นแวมไพร์ แลกกับที่ดินทำเลทองส่วนหนึ่งในแถบอังกฤษให้กับวินยาร์ด<br><br>บารอนวินยาร์ดและอาเธอร์ตอบรับข้อเสนอนี้ พวกเขาได้เดินทางกลับไปยังคฤหาสน์ไคเซอร์ทันทีเพื่อทำการถ่ายเลือดให้กับมิคาเอล ทำให้มิคาเอลกลับมามีชีวิตอีกครั้ง มิคาเอลกลับมาพร้อมชีวิตอันเป็นอมตะและพละกำลังที่เหนือมนุษย์ ทว่าเขากลับอยู่กลางแดดนาน ๆ ไม่ได้ มิคาเอลจึงได้ใช้เวลากลางวันหมกตัวอยู่แต่ในคฤหาสน์ ส่วนกลางคืนเขาจะออกไปช่วยงานในกิจการของครอบครัว มิคาเอลเป็นชายหนุ่มที่มีจิตใจดี และค่อนข้างที่จะอ่อนไหวง่ายจากการประคบประหงมของครอบครัว เขาจึงเลือกที่จะดื่มเลือดสัตว์แทนเลือดมนุษย์ เมื่อกาลเวลาผันผ่านไปเอิร์ลและเคาท์เตสไคเซอร์ได้สิ้นใจด้วยโรคชรา มิคาเอลที่มีชีวิตอมตะทนไม่ได้ที่จะเห็นคนรอบตัวที่ตัวเองรักจากไปทีละคน เขาจึงเลือกที่จะย้ายออกจากคฤหาสน์ของตัวเองและสละตำแหน่งเอิร์ลแห่งไคเซอร์ให้กับญาติที่ไว้ใจได้ผู้หนึ่ง แต่ก็เพียงแค่ในนามเท่านั้น เพราะมิคาเอลนั้นไม่ยอมให้สิ่งที่พ่อและแม่หามาอย่างยากลำบากนั้นต้องตกไปให้ผู้อื่นมาถลุงเล่น<br><br>หลังจากนั้นตัวเขาก็ได้เดินทางเปลี่ยนที่อยู่ไปเรื่อย ๆ ก่อนที่จะพบเข้ากับหมู่บ้านเอลิเชียนในช่วงต้นศตวรรษที่ 20 เขาจึงได้ตัดสินใจที่จะย้ายมาใช้ชีวิตที่หมู่บ้านเอลิเชียนอย่างถาวร แต่แล้วก็มีเหตุที่ทำให้เขานั้นต้องออกจากหมู่บ้านไปนานเป็นสิบ ๆ ปี เพื่อตามหาหลานชายที่กลายเป็นแวมไพร์ที่เขาไม่เคยรู้ว่ามีอย่างเอสเตแวน เมื่อหาจนพลิกแผ่นดินก็ไม่พบ มิคาเอลจึงได้เลือกที่จะกลับมาปักหลักที่หมู่บ้านเอลิเชียนอีกครั้งในปี 2024 และได้สมรสกับ ลิลิธ พริมูแวร์ แซงกวิสิค (สกุลเดิม) เมื่อวันคริสต์มาสในปีเดียวกัน ซึ่งทั้งคู่นั้นเคยพบกันที่อังกฤษมาก่อน แต่สุดท้ายก็ได้มาตกหลุมรักกันที่เอลิเชียนและตกลงแต่งงานเป็นคู่สามีภรรยาในที่สุด</p><div class=\"ddsh-commtmklpfo-inline-redaction\"><span></span><span></span><span class=\"short\"></span></div></div></section><section class=\"ddsh-commtmklpfo-section\"><div class=\"ddsh-commtmklpfo-sectionhead\"><strong>02</strong><div><span>BEHAVIORAL PROFILE</span><h2>PERSONALITY</h2></div><small>PERSONALITY REPORT</small></div><div class=\"ddsh-commtmklpfo-text\"><div class=\"ddsh-commtmklpfo-recordline\"><span>ASSESSMENT:</span><strong>OBSERVATIONAL PROFILE</strong><span>STATUS:</span><strong>VERIFIED</strong></div><p>เป็นคนที่จิตใจดี ฉลาด แต่ค่อนข้างเก็บตัว ไม่ค่อยได้ออกมาพบปะกับผู้คนมากนัก แต่ถ้าได้สนิทกับใครเขามักจะเป็นฝ่ายชวนพูดเสียมากกว่า ในเวลาว่างมักจะหมกตัวอ่านหนังสือและทำเพลงส่งให้ค่ายเพลงที่บ้าน มักจะไม่ค่อยอยากสนิทสนมกับผู้อื่นมากนักเนื่องจากเขากลัวการสูญเสียเหมือนที่เคยสูญเสียครอบครัวและคนที่รักคนอื่น ๆ รักภรรยามาก และมักจะตามใจเธอเสมอ หลังจากย้ายมาอยู่หมู่บ้านเอลิเชียน มิคาเอลก็ได้เปิดใจคบคนอื่นเป็นเพื่อนบ้างแล้ว</p><p>Additional behavioral notes: <span class=\"ddsh-redact ddsh-redact-md\">REDACTED</span> pending further departmental assessment.</p><div class=\"ddsh-commtmklpfo-classified-note\"><span>CLASSIFIED NOTE</span><div><b></b><b></b><b class=\"half\"></b></div></div></div></section><section class=\"ddsh-commtmklpfo-section\"><div class=\"ddsh-commtmklpfo-sectionhead\"><strong>03</strong><div><span>ASSOCIATED PERSONS</span><h2>RELATIONSHIPS</h2></div><small>RELATIONSHIP RECORD</small></div><div class=\"ddsh-commtmklpfo-relations\"><div class=\"ddsh-commtmklpfo-relation\"><div class=\"ddsh-commtmklpfo-rel-no\">01</div><div class=\"ddsh-commtmklpfo-rel-name\"><span>NAME</span><strong>Lilith P. Kaiser</strong></div><div class=\"ddsh-commtmklpfo-rel-type\"><span>RELATIONSHIP</span><strong>Wife</strong></div></div><div class=\"ddsh-commtmklpfo-relation\"><div class=\"ddsh-commtmklpfo-rel-no\">02</div><div class=\"ddsh-commtmklpfo-rel-name\"><span>NAME</span><strong>Flanklin D. Bloodworth</strong></div><div class=\"ddsh-commtmklpfo-rel-type\"><span>RELATIONSHIP</span><strong>BFF</strong></div></div></div></section><div class=\"ddsh-commtmklpfo-bottom\"><div class=\"ddsh-commtmklpfo-custody\"><strong>CONFIDENTIAL</strong><span>AUTHORIZED PERSONNEL ONLY</span></div><div class=\"ddsh-commtmklpfo-bottomredact\"><span>ADDITIONAL NOTES / REDACTIONS</span><div><i></i><i></i><i class=\"short\"></i></div></div></div><div class=\"ddsh-commtmklpfo-footer\"><div><span>CLASSIFICATION</span><strong>CONFIDENTIAL — PERSONAL RECORD</strong></div><div class=\"ddsh-commtmklpfo-footer-code\">EPD / RECORDS DIVISION / 0023-44</div><div class=\"ddsh-commtmklpfo-footerstamp\">NOT FOR PUBLIC RELEASE</div></div></div></div><div class=\"ddshcr-ccmmkl1\"><span></span></div>";
  ...
  function recordCode(v) {
    const left = digits(v.record4,4);
    const right = digits(v.record2,2);
  ...
  function styleVars(v) {
    ...
    const stamp = /^#[0-9a-f]{6}$/i.test(String(v.stampColor || "")) ? v.stampColor : defaults.stampColor;
    const safePhoto = String(v.photo || defaults.photo).replace(/'/g, "%27");
    const photoX = clampNumber(v.photoX, -300, 300, 0);
    const photoY = clampNumber(v.photoY, -300, 300, 0);
    const photoZoom = clampNumber(v.photoZoom, 50, 200, 100);
    return [
      ...
      `--ddsh-commtmklpfo-red:${main}`,
      `--ddsh-commtmklpfo-stamp:${stamp}`,
      "--ddsh-commtmklpfo-logo:url('https://iili.io/nxjgK22.png')",
      `--ddsh-commtmklpfo-photo:url('${safePhoto}')`,
      `--ddsh-commtmklpfo-photo-x:${photoX}px`,
      `--ddsh-commtmklpfo-photo-y:${photoY}px`,
      `--ddsh-commtmklpfo-photo-zoom:${(photoZoom / 100).toFixed(3)}`,
  ...
  function buildCode(v = defaults) {
    return `<link href="${CSS_URL}" rel="stylesheet"><link href="${FONT_URL}" rel="stylesheet"><style data-mhp-final-colors>.ddsh-commtmklpfofile .ddsh-commtmklpfo-barcode{background-color:transparent!important;background-image:repeating-linear-gradient(90deg,var(--ddsh-commtmklpfo-ink) 0 2px,transparent 2px 4px,var(--ddsh-commtmklpfo-ink) 4px 5px,transparent 5px 8px,var(--ddsh-commtmklpfo-ink) 8px 11px,transparent 11px 13px,var(--ddsh-commtmklpfo-ink) 13px 14px,transparent 14px 18px)!important;border-color:var(--ddsh-commtmklpfo-ink)!important;box-shadow:none!important;filter:none!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-barcode::before,.ddsh-commtmklpfofile .ddsh-commtmklpfo-barcode::after{background:var(--ddsh-commtmklpfo-ink)!important;background-color:var(--ddsh-commtmklpfo-ink)!important;border-color:var(--ddsh-commtmklpfo-ink)!important;color:var(--ddsh-commtmklpfo-ink)!important;box-shadow:none!important;filter:none!important}.ddsh-commtmklpfofile .ddsh-blackline,.ddsh-commtmklpfofile .ddsh-mini-redact,.ddsh-commtmklpfofile .ddsh-redact,.ddsh-commtmklpfofile .ddsh-commtmklpfo-inline-redaction span,.ddsh-commtmklpfofile .ddsh-commtmklpfo-classified-note b,.ddsh-commtmklpfofile .ddsh-commtmklpfo-bottomredact i{background:var(--ddsh-commtmklpfo-ink)!important;background-color:var(--ddsh-commtmklpfo-ink)!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-redstamp,.ddsh-commtmklpfofile .ddsh-commtmklpfo-redstamp strong,.ddsh-commtmklpfofile .ddsh-commtmklpfo-redstamp span,.ddsh-commtmklpfofile .ddsh-commtmklpfo-redstamp::before,.ddsh-commtmklpfofile .ddsh-commtmklpfo-redstamp::after{color:var(--ddsh-commtmklpfo-stamp)!important;border-color:var(--ddsh-commtmklpfo-stamp)!important;box-shadow:none!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-custody,.ddsh-commtmklpfofile .ddsh-commtmklpfo-custody strong,.ddsh-commtmklpfofile .ddsh-commtmklpfo-custody span,.ddsh-commtmklpfofile .ddsh-commtmklpfo-custody::before,.ddsh-commtmklpfofile .ddsh-commtmklpfo-custody::after{color:var(--ddsh-commtmklpfo-stamp)!important;border-color:var(--ddsh-commtmklpfo-stamp)!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-footerstamp,.ddsh-commtmklpfofile .ddsh-commtmklpfo-footerstamp::before,.ddsh-commtmklpfofile .ddsh-commtmklpfo-footerstamp::after{color:var(--ddsh-commtmklpfo-ink)!important;border-color:var(--ddsh-commtmklpfo-ink)!important;box-shadow:none!important}.ddsh-commtmklpfofile .ddsh-commtmklpfo-photo::before{transform:translate(var(--ddsh-commtmklpfo-photo-x,0px),var(--ddsh-commtmklpfo-photo-y,0px)) scale(var(--ddsh-commtmklpfo-photo-zoom,1))!important;transform-origin:center center!important}</style><div class="ddsh-commtmklpfofile" style="${styleVars(v)}"><div class="ddsh-commtmklpfo-paperfx"></div><div class="ddsh-commtmklpfo-content"><header class="ddsh-commtmklpfo-header"><div class="ddsh-commtmklpfo-dept"><div class="ddsh-commtmklpfo-police-logo"></div><div class="ddsh-commtmklpfo-depttext"><span>ELYSIAN POLICE DEPARTMENT</span><strong>RECORDS & INVESTIGATION DIVISION</strong><small>CONFIDENTIAL PERSONNEL ARCHIVE</small></div></div><div class="ddsh-commtmklpfo-title"><h1>CASE FILE</h1><span>OFFICIAL PERSONNEL RECORD</span></div><div class="ddsh-commtmklpfo-number"><div class="ddsh-commtmklpfo-numbertop"><span>CASE NO.</span><strong>EPD-${digits(v.caseYear,2)}-${digits(v.caseNo,4)}</strong></div><div class="ddsh-commtmklpfo-barcode"></div><div class="ddsh-commtmklpfo-barcode-number">8 920176 ${digits(v.record4,4)}${digits(v.record2,2)}</div><small>RESTRICTED DATABASE RECORD</small></div></header><div class="ddsh-commtmklpfo-heavyline"></div><section class="ddsh-commtmklpfo-main"><div class="ddsh-commtmklpfo-photos"><div class="ddsh-commtmklpfo-photo"><div class="ddsh-commtmklpfo-height"><span>190</span><span>180</span><span>170</span><span>160</span><span>150</span></div><div class="ddsh-commtmklpfo-photo-id"><strong>EPD</strong><span>SUBJECT VIEW</span><b>${recordCode(v)}</b></div></div></div><div class="ddsh-commtmklpfo-identification"><div class="ddsh-commtmklpfo-blocktitle"><span>SUBJECT IDENTIFICATION</span><b>FILE: ACTIVE</b></div><div class="ddsh-commtmklpfo-name"><span>FULL NAME (THAI)</span><strong>${esc(v.fullName)}</strong><small>LEGAL IDENTITY CONFIRMED</small></div><div class="ddsh-commtmklpfo-info"><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">ALIASES</span><span class="ddsh-commtmklpfo-value">${esc(v.aliases)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">FACECLAIM</span><span class="ddsh-commtmklpfo-value">${esc(v.faceclaim)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">BIRTHPLACE</span><span class="ddsh-commtmklpfo-value">${esc(v.birthplace)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">DATE OF BIRTH</span><span class="ddsh-commtmklpfo-value">${esc(v.dateOfBirth)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">SPECIES</span><span class="ddsh-commtmklpfo-value">${esc(v.species)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">LEVEL</span><span class="ddsh-commtmklpfo-value">${esc(v.level)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">AGE</span><span class="ddsh-commtmklpfo-value">${esc(v.age)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">MBTI</span><span class="ddsh-commtmklpfo-value">${esc(v.mbti)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">EDUCATION</span><span class="ddsh-commtmklpfo-value">${esc(v.education)}</span></div><div class="ddsh-commtmklpfo-row"><span class="ddsh-commtmklpfo-label">OCCUPATION</span><span class="ddsh-commtmklpfo-value">${esc(v.occupation)}</span></div></div><div class="ddsh-commtmklpfo-subject-note"><div class="ddsh-commtmklpfo-subject-note-head"><span>NOTE</span><b>SUBJECT REMARK</b></div><p>${multiline(v.note)}</p></div></div><aside class="ddsh-commtmklpfo-side"><div class="ddsh-commtmklpfo-sidebox"><span>CASE STATUS</span><strong>ACTIVE FILE</strong><small>RECORD UNDER REVIEW</small></div><div class="ddsh-commtmklpfo-sidebox"><span>SECURITY LEVEL</span><strong>CLASSIFIED</strong><small>AUTHORIZED PERSONNEL ONLY</small></div><div class="ddsh-commtmklpfo-sidebox"><span>DATABASE STATUS</span><strong>VERIFIED</strong><small>LAST UPDATE: 05/29/2026</small></div><div class="ddsh-commtmklpfo-sidebox"><span>REFERENCE</span><strong>EPD-${digits(v.reference4,4)}</strong><small><span class="ddsh-redact ddsh-redact-long">REDACTED DATA</span></small></div><div class="ddsh-commtmklpfo-redstamp"><strong>ARCHIVED</strong><span>PERSONAL RECORD</span></div></aside></section><div class="ddsh-commtmklpfo-photo-caption"><span>SUBJECT IMAGE / IDENTIFICATION RECORD NO. EPD-${recordCode(v)}</span><span>IMAGE SOURCE: <b class="ddsh-mini-redact"></b></span></div><section class="ddsh-commtmklpfo-chargebox"><div class="ddsh-commtmklpfo-charge-title">FILE INFORMATION</div><div class="ddsh-commtmklpfo-charge-grid"><div><span>RECORD TYPE</span><strong>PERSONAL HISTORY</strong></div><div><span>DOCUMENT STATUS</span><strong>COMPLETE RECORD</strong></div><div><span>ACCESS LEVEL</span><strong>CONFIDENTIAL</strong></div><div><span>FILE OWNER</span><strong class="ddsh-blackline"></strong></div></div></section><section class="ddsh-commtmklpfo-section"><div class="ddsh-commtmklpfo-sectionhead"><strong>01</strong><div><span>BACKGROUND RECORD</span><h2>HISTORY</h2></div><small>SUBJECT HISTORY</small></div><div class="ddsh-commtmklpfo-text"><div class="ddsh-commtmklpfo-recordline"><span>REPORTING OFFICER:</span><b class="ddsh-blackline ddsh-blackline-md"></b><span>DATE:</span><strong>${esc(v.reportDate)}</strong></div><p>${multiline(v.history)}</p><div class="ddsh-commtmklpfo-inline-redaction"><span></span><span></span><span class="short"></span></div></div></section><section class="ddsh-commtmklpfo-section"><div class="ddsh-commtmklpfo-sectionhead"><strong>02</strong><div><span>BEHAVIORAL PROFILE</span><h2>PERSONALITY</h2></div><small>PERSONALITY REPORT</small></div><div class="ddsh-commtmklpfo-text"><div class="ddsh-commtmklpfo-recordline"><span>ASSESSMENT:</span><strong>OBSERVATIONAL PROFILE</strong><span>STATUS:</span><strong>VERIFIED</strong></div><p>${multiline(v.personality)}</p><p>Additional behavioral notes: <span class="ddsh-redact ddsh-redact-md">REDACTED</span> pending further departmental assessment.</p><div class="ddsh-commtmklpfo-classified-note"><span>CLASSIFIED NOTE</span><div><b></b><b></b><b class="half"></b></div></div></div></section><section class="ddsh-commtmklpfo-section"><div class="ddsh-commtmklpfo-sectionhead"><strong>03</strong><div><span>ASSOCIATED PERSONS</span><h2>RELATIONSHIPS</h2></div><small>RELATIONSHIP RECORD</small></div><div class="ddsh-commtmklpfo-relations">${relationsHtml(v)}</div></section><div class="ddsh-commtmklpfo-bottom"><div class="ddsh-commtmklpfo-custody"><strong>CONFIDENTIAL</strong><span>AUTHORIZED PERSONNEL ONLY</span></div><div class="ddsh-commtmklpfo-bottomredact"><span>ADDITIONAL NOTES / REDACTIONS</span><div><i></i><i></i><i class="short"></i></div></div></div><div class="ddsh-commtmklpfo-footer"><div><span>CLASSIFICATION</span><strong>CONFIDENTIAL — PERSONAL RECORD</strong></div><div class="ddsh-commtmklpfo-footer-code">EPD / RECORDS DIVISION / ${recordCode(v)}</div><div class="ddsh-commtmklpfo-footerstamp">NOT FOR PUBLIC RELEASE</div></div></div></div><div class="ddshcr-ccmmkl1"><span></span></div>`;
  }
  ...
  function fitViewPreview() {
    const iframe = viewPanel?.querySelector("[data-mhp-view-preview]");
    const stage = viewPanel?.querySelector("[data-mhp-view-stage]");
    if (!iframe || !stage || stage.clientWidth < 20) return;
    const { width, height } = measureIframe(iframe);
    const padding = 24;
    const scale = Math.max(.01, Math.min(1, (stage.clientWidth - padding * 2) / width));
  ...
  function rangeField(label, key, value, min, max, step = 1, unit = "") {
    return `<label class="dds-field dds-field-full dds-mhp-range-field"><span>${label}</span><div class="dds-mhp-range-row"><input type="range" name="mhp-${key}" data-mhp-field="${key}" min="${min}" max="${max}" step="${step}" value="${attr(value)}"><output data-mhp-range-output="${key}">${esc(value)}${unit}</output></div></label>`;
  }
  ...
  function createPanel() {
    ...
    panel.dataset.panel = PANEL_NAME;
    panel.innerHTML = `<div class="dds-editor-heading"><button aria-label="กลับหน้า COMMISSION" class="dds-back-button" data-mhp-back type="button">←</button><div><p class="dds-eyebrow">COMMISSION CODE EDITOR</p><h1 class="dds-mhp-editor-heading"><span>COMMISSION</span><span>— โคดประเภทประวัติ</span></h1><p>ทุกคนใช้ได้ฟรี · ผู้จ้าง MIKAEL F. KAISER</p></div></div><div class="dds-protected-commission-layout"><div class="dds-protected-commission-preview-column"><div class="dds-editor-preview-top"><span>LIVE PREVIEW</span><strong>MIKAEL HISTORY / FREE</strong></div><div class="dds-protected-commission-preview-stage dds-mhp-preview-stage" data-mhp-preview-stage><iframe class="dds-protected-commission-preview-frame dds-mhp-preview-frame" data-mhp-preview scrolling="no" title="พรีวิวโคดประวัติ Mikael"></iframe></div></div><div class="dds-protected-commission-controls-column dds-mhp-controls-column"><div class="dds-protected-commission-draft dds-mhp-draft"><div><strong>บันทึกแบบร่าง</strong><small data-mhp-draft-status>ยังไม่มีแบบร่าง</small></div></div><div class="dds-protected-commission-scroll dds-mhp-controls-scroll"><section class="dds-control-section"><div class="dds-control-title"><span>01</span><h2>สีของโคด</h2></div><div class="dds-color-grid">${colorField("สีพื้นหลัง","paperColor",defaults.paperColor)}${colorField("สีหลักทั้งหมด (ข้อความ / เส้น / ขีดทึบ / Barcode)","mainColor",defaults.mainColor)}${colorField("สีแสตมป์ทั้ง 2 ตำแหน่ง","stampColor",defaults.stampColor)}</div></section><section class="dds-control-section"><div class="dds-control-title"><span>02</span><h2>รูปภาพ</h2></div><div class="dds-form-grid">${field("URL รูปหลัก","photo",defaults.photo,true)}${rangeField("เลื่อนรูป ซ้าย ↔ ขวา","photoX",defaults.photoX,-300,300,1,"px")}${rangeField("เลื่อนรูป ขึ้น ↕ ลง","photoY",defaults.photoY,-300,300,1,"px")}${rangeField("ซูมรูป เข้า / ออก","photoZoom",defaults.photoZoom,50,200,1,"%")}</div></section><section class="dds-control-section"><div class="dds-control-title"><span>03</span><h2>เลขแฟ้ม</h2></div><div class="dds-form-grid">${field("เลขแฟ้มบนรูป / เอกสาร (4 ตัว)","record4",defaults.record4,false,'inputmode="numeric" maxlength="4"')}${field("เลขท้ายแฟ้ม (2 ตัว)","record2",defaults.record2,false,'inputmode="numeric" maxlength="2"')}${field("เลขปี CASE NO. (2 ตัว)","caseYear",defaults.caseYear,false,'inputmode="numeric" maxlength="2"')}${field("เลขคดี CASE NO. (4 ตัว)","caseNo",defaults.caseNo,false,'inputmode="numeric" maxlength="4"')}${field("เลข REFERENCE EPD- (4 ตัว)","reference4",defaults.reference4,false,'inputmode="numeric" maxlength="4"')}${field("วันที่ในเอกสาร (MM/DD/YYYY)","reportDate",defaults.reportDate)}</div></section><section class="dds-control-section"><div class="dds-control-title"><span>04</span><h2>SUBJECT IDENTIFICATION</h2></div><div class="dds-form-grid">${field("FULL NAME (THAI)","fullName",defaults.fullName,true)}${field("ALIASES","aliases",defaults.aliases)}${field("FACECLAIM","faceclaim",defaults.faceclaim)}${field("BIRTHPLACE","birthplace",defaults.birthplace)}${field("DATE OF BIRTH","dateOfBirth",defaults.dateOfBirth)}${field("SPECIES","species",defaults.species)}${field("LEVEL","level",defaults.level)}${field("AGE","age",defaults.age)}${field("MBTI","mbti",defaults.mbti)}${field("EDUCATION","education",defaults.education,true)}${field("OCCUPATION","occupation",defaults.occupation,true)}${area("NOTE","note",defaults.note,3)}</div></section><section class="dds-control-section"><div class="dds-control-title"><span>05</span><h2>HISTORY / PERSONALITY</h2></div><div class="dds-form-grid">${area("HISTORY","history",defaults.history,12)}${area("PERSONALITY","personality",defaults.personality,8)}</div></section><section class="dds-control-section"><div class="dds-control-title"><span>06</span><h2>RELATIONSHIPS</h2></div><div class="dds-mhp-relation-count"><label class="dds-field"><span>จำนวนคนที่ต้องการแสดง</span><input type="number" name="mhp-relationCount" data-mhp-field="relationCount" min="1" max="${MAX_RELATIONS}" value="1"></label><div class="dds-mhp-relation-count-actions"><button type="button" data-mhp-relation-minus>− REMOVE</button><button type="button" data-mhp-relation-plus>+ ADD</button></div></div><div class="dds-mhp-relations-editor-list">${relationEditorRows()}</div></section></div><section class="dds-control-section dds-copy-section dds-mhp-copy-section"><div class="dds-control-title"><span>07</span><h2>คัดลอกโคด</h2></div><div class="dds-editor-actions"><button class="dds-copy-button" data-mhp-copy type="button">COPY CODE <span>↗</span></button><button class="dds-reset-button" data-mhp-reset type="button">RESET</button></div></section></div></div>`;
    document.querySelector(".dds-main")?.appendChild(panel);
    bindPanel();
    return panel;
  }
  ...
  function createViewPanel() {
    ...
    if (!main) return null;
    viewPanel = document.createElement("section");
    viewPanel.className = "dds-panel dds-commission-view-panel dds-mhp-view-panel";
    viewPanel.dataset.panel = VIEW_PANEL_NAME;
    viewPanel.innerHTML = `<div class="dds-commission-view-toolbar"><button aria-label="กลับหน้า COMMISSION & SHOWCASE" class="dds-back-button" data-mhp-view-back type="button">←</button></div><div class="dds-mhp-view-stage" data-mhp-view-stage><iframe class="dds-editor-preview-frame dds-commission-view-frame dds-mhp-view-frame" data-mhp-view-preview scrolling="no" title="งาน COMMISSION โคดประเภทประวัติ MIKAEL F. KAISER"></iframe></div>`;
    if (footer?.parentElement === main) main.insertBefore(viewPanel, footer);
    else main.appendChild(viewPanel);
    viewPanel.querySelector("[data-mhp-view-back]")?.addEventListener("click", goBack);
    return viewPanel;
  }
  ...
  function openView() {
    ...
    if (!target) return;
    showPanel(VIEW_PANEL_NAME);
    history.replaceState(null, "", "#view-commission-mikael-history-free");
    writeIframe(target.querySelector("[data-mhp-view-preview]"), OFFICIAL_CODE, fitViewPreview);
  }
  ...
  function setValues(values) {
    ...
    panel?.querySelectorAll("[data-mhp-range-output]").forEach(output => {
      const key = output.dataset.mhpRangeOutput;
      const input = panel.querySelector(`[data-mhp-field="${key}"]`);
      if (!input) return;
      output.textContent = `${input.value}${key === "photoZoom" ? "%" : "px"}`;
    });
    syncRelationRows();
  }
  ...
  function bindPanel() {
    ...
    panel.querySelectorAll("[data-mhp-field]").forEach(input => {
      const onChange = () => {
        ...
        if (picker && /^#[0-9a-f]{6}$/i.test(input.value.trim())) picker.value = input.value.trim();
        const rangeOutput = panel.querySelector(`[data-mhp-range-output="${input.dataset.mhpField}"]`);
        if (rangeOutput) {
          const unit = input.dataset.mhpField === "photoZoom" ? "%" : "px";
          rangeOutput.textContent = `${input.value}${unit}`;
        }
        updatePreview();
        scheduleDraft();
      };
  ...
  function showPanel(name) {
    ...
    const focused = editing || name === VIEW_PANEL_NAME;
    document.body.classList.toggle("dds-editor-mode", focused);
    document.body.classList.toggle("dds-commission-editor-mode", editing);
    document.documentElement.classList.toggle("dds-editor-mode", focused);
  ...
  function handleHash() {
    if (location.hash === "#editor-commission-mikael-history-free") openEditor();
    if (location.hash === "#view-commission-mikael-history-free") openView();
  }
  ...
  function install() {
    ...
    window.addEventListener("resize", () => {
      ...
      if (viewPanel?.classList.contains("is-active")) fitViewPreview();
    });
