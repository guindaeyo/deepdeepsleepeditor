...
const commissionViewButtons = document.querySelectorAll("[data-view-commission]");
...
const pageTitles = {
  ...
  "editor-profile002": "― www. deep deep sleep code shop .com ―",
...
function openPage(pageName, updateHash = true) {
    ...
    const navPage =
      validPage.startsWith("editor-review")
        ...
        : validPage.startsWith("editor-profile")
          ? "profile"
          : validPage.startsWith("editor-commission")
            ? "commission"
            : validPage.startsWith("editor-")
              ? "roleplay"
              : validPage;
  ...
  if (validPage === "editor-profile002") {
    updateMoodboard();
  }

  if (validPage === "editor-profile003") {
    updateFortyOne();
  }

  if (validPage === "editor-profile004") {
    updateNothinBoutMe();
  }
  ...
  if (validPage === "editor-commission001") {
...
profileEditButtons.forEach((button) => {
  button.addEventListener("click", () => {
    ...
    if (profileName === "profile002") {
      openPage("editor-profile002");
    }

    if (profileName === "profile003") {
      openPage("editor-profile003");
    }

    if (profileName === "profile004") {
      openPage("editor-profile004");
    }
  });
});
...
commissionViewButtons.forEach((button) => {
  button.addEventListener("click", () => {
    const commissionName =
      button.dataset.viewCommission;
...
function measureMoodboardCanvas(iframe) {
  const previewDocument =
    iframe?.contentDocument;
...
function resizeMoodboardDynamicPreview(iframe) {
  if (!iframe) {
    return;
  }
  ...
  const stage =
    iframe.closest(
      ".dds-moodboard-preview-stage, .dds-roleplay-card-preview"
    );
  ...
  const isEditorPreview =
    iframe.classList.contains(
      "dds-moodboard-editor-preview-frame"
    );
  ...
  if (isEditorPreview) {
    const zoom =
      Math.max(
        ...
        Math.min(
          2.5,
          Number(
            iframe.dataset.moodboardZoom || 1
          ) || 1
        )
      );
    ...
    const host =
      iframe.closest(
        ".dds-moodboard-preview-canvas-host"
      );
    ...
    iframe.style.setProperty(
      "--dds-moodboard-canvas-scale",
      String(scale)
    );

    updateMoodboardZoomOutput();
...
function updateMoodboardZoomOutput() {
  const iframe =
    document.querySelector(
      "#moodboardPreview"
    );
...
function setMoodboardZoom(nextZoom) {
  const iframe =
    document.querySelector(
      "#moodboardPreview"
    );
  ...
  iframe.dataset.moodboardZoom =
    zoom.toFixed(2);

  resizeMoodboardDynamicPreview(
    iframe
  );
...
function resizeFortyOneFixedPreview(iframe) {
  if (!iframe) {
    return;
  }
  ...
  const stage =
    iframe.closest(
      ".dds-fortyone-preview-stage, .dds-roleplay-card-preview"
    );
  ...
  const canvasHeight =
    Math.max(
      ...
      Number(
        iframe.dataset
          .fortyoneCanvasHeight || 1700
      ) || 1700
    );

  const isEditorPreview =
    iframe.classList.contains(
      "dds-fortyone-editor-preview-frame"
    );
  ...
  if (isEditorPreview) {
    ...
    const host =
      iframe.closest(
        ".dds-fortyone-preview-canvas-host"
      );
    ...
    iframe.style.setProperty(
      "--dds-fortyone-canvas-scale",
      String(scale)
    );

    updateFortyOneZoomOutput();
...
function updateFortyOneZoomOutput() {
  const iframe =
    document.querySelector(
      "#fortyOnePreview"
    );
...
function setFortyOneZoom(nextZoom) {
  const iframe =
    document.querySelector(
      "#fortyOnePreview"
    );
  ...
  iframe.dataset.fortyoneZoom =
    zoom.toFixed(2);

  resizeFortyOneFixedPreview(
    iframe
  );
...
function resizeNothinFixedPreview(iframe) {
  if (!iframe) {
    return;
  }
  ...
  const stage =
    iframe.closest(
      ".dds-nothin-preview-stage, .dds-roleplay-card-preview"
    );
  ...
  const canvasHeight =
    Math.max(
      ...
      Number(
        iframe.dataset
          .nothinCanvasHeight || 1000
      ) || 1000
    );

  const isEditorPreview =
    iframe.classList.contains(
      "dds-nothin-editor-preview-frame"
    );
  ...
  if (isEditorPreview) {
    ...
    const host =
      iframe.closest(
        ".dds-nothin-preview-canvas-host"
      );
    ...
    iframe.style.setProperty(
      "--dds-nothin-canvas-scale",
      String(scale)
    );

    updateNothinZoomOutput();
...
function updateNothinZoomOutput() {
  const iframe =
    document.querySelector(
      "#nothinPreview"
    );
...
function setNothinZoom(nextZoom) {
  const iframe =
    document.querySelector(
      "#nothinPreview"
    );
  ...
  iframe.dataset.nothinZoom =
    zoom.toFixed(2);

  resizeNothinFixedPreview(
    iframe
  );
...
function resizeCommissionFixedPreview(iframe) {
  ...
  /*
   * หน้าที่ถูกซ่อนมี clientWidth/clientHeight เป็น 0
   * ห้ามคำนวณ scale จากค่านี้ เพราะจะทำให้พรีวิวจิ๋ว
   */
  if (
    stageWidth < 20 ||
    stageHeight < 20
  ...
  if (isFullView) {
    /*
     * บนคอมแสดง post column 734px แบบ 1:1
     * บนหน้าจอแคบจึงค่อยย่อให้พอดี
     */
    scale =
      Math.min(
        1,
        (stageWidth - 2) / postWidth,
        (stageHeight - 2) / postHeight
      );
  } else {
    scale =
      Math.min(
        (stageWidth - 18) / postWidth,
        (stageHeight - 18) / postHeight
      );
  }
...
const profileCardPreview002 =
  document.querySelector(
    "#profileCardPreview002"
  );

const generatedMoodboardCode =
  document.querySelector(
    "#generatedMoodboardCode"
  );

const fieldsProfile002 = {
  bgColor: document.querySelector("#moodBgColor"),
  frameBgColor: document.querySelector("#moodFrameBgColor"),
  textColor: document.querySelector("#moodTextColor"),
  miniColor: document.querySelector("#moodMiniColor"),
  titleColor: document.querySelector("#moodTitleColor"),
  tagTextColor: document.querySelector("#moodTagTextColor"),
  oliveColor: document.querySelector("#moodOliveColor"),
  orangeColor: document.querySelector("#moodOrangeColor"),
  musicBgColor: document.querySelector("#moodMusicBgColor"),
  borderColor: document.querySelector("#moodBorderColor"),
  accentTextColor: document.querySelector("#moodAccentTextColor"),

  dateText: document.querySelector("#moodDateText"),
  searchText: document.querySelector("#moodSearchText"),
  contactText: document.querySelector("#moodContactText"),
  miniCaption: document.querySelector("#moodMiniCaption"),
  mainTitle: document.querySelector("#moodMainTitle"),

  leftImageOne: document.querySelector("#moodLeftImageOne"),
  leftImageOneX: document.querySelector("#moodLeftImageOneX"),
  leftImageOneY: document.querySelector("#moodLeftImageOneY"),

  leftImageTwo: document.querySelector("#moodLeftImageTwo"),
  leftImageTwoX: document.querySelector("#moodLeftImageTwoX"),
  leftImageTwoY: document.querySelector("#moodLeftImageTwoY"),

  centerImageOne: document.querySelector("#moodCenterImageOne"),
  centerImageOneX: document.querySelector("#moodCenterImageOneX"),
  centerImageOneY: document.querySelector("#moodCenterImageOneY"),

  centerImageTwo: document.querySelector("#moodCenterImageTwo"),
  centerImageTwoX: document.querySelector("#moodCenterImageTwoX"),
  centerImageTwoY: document.querySelector("#moodCenterImageTwoY"),

  tagOne: document.querySelector("#moodTagOne"),
  tagTwo: document.querySelector("#moodTagTwo"),
  tagThree: document.querySelector("#moodTagThree"),
  tagFour: document.querySelector("#moodTagFour"),
  tagFive: document.querySelector("#moodTagFive"),
  tagSix: document.querySelector("#moodTagSix"),

  rightImageOne: document.querySelector("#moodRightImageOne"),
  rightImageOneX: document.querySelector("#moodRightImageOneX"),
  rightImageOneY: document.querySelector("#moodRightImageOneY"),

  rightImageTwo: document.querySelector("#moodRightImageTwo"),
  rightImageTwoX: document.querySelector("#moodRightImageTwoX"),
  rightImageTwoY: document.querySelector("#moodRightImageTwoY"),

  placeTitle: document.querySelector("#moodPlaceTitle"),
  placeText: document.querySelector("#moodPlaceText"),

  placeAvatar: document.querySelector("#moodPlaceAvatar"),
  placeAvatarX: document.querySelector("#moodPlaceAvatarX"),
  placeAvatarY: document.querySelector("#moodPlaceAvatarY"),

  placeImageOne: document.querySelector("#moodPlaceImageOne"),
  placeImageOneX: document.querySelector("#moodPlaceImageOneX"),
  placeImageOneY: document.querySelector("#moodPlaceImageOneY"),

  placeImageTwo: document.querySelector("#moodPlaceImageTwo"),
  placeImageTwoX: document.querySelector("#moodPlaceImageTwoX"),
  placeImageTwoY: document.querySelector("#moodPlaceImageTwoY"),

  musicCover: document.querySelector("#moodMusicCover"),
  musicCoverX: document.querySelector("#moodMusicCoverX"),
  musicCoverY: document.querySelector("#moodMusicCoverY"),
  songTitle: document.querySelector("#moodSongTitle"),
  artistName: document.querySelector("#moodArtistName"),

  collageTop: document.querySelector("#moodCollageTop"),
  collageTopX: document.querySelector("#moodCollageTopX"),
  collageTopY: document.querySelector("#moodCollageTopY"),

  collageIris: document.querySelector("#moodCollageIris"),
  collageIrisX: document.querySelector("#moodCollageIrisX"),
  collageIrisY: document.querySelector("#moodCollageIrisY"),

  collagePortrait: document.querySelector("#moodCollagePortrait"),
  collagePortraitX: document.querySelector("#moodCollagePortraitX"),
  collagePortraitY: document.querySelector("#moodCollagePortraitY"),

  collageFlower: document.querySelector("#moodCollageFlower"),
  collageFlowerX: document.querySelector("#moodCollageFlowerX"),
  collageFlowerY: document.querySelector("#moodCollageFlowerY"),

  bottomPill: document.querySelector("#moodBottomPill"),
  bottomSymbols: document.querySelector("#moodBottomSymbols"),
  displayName: document.querySelector("#moodDisplayName"),
  description: document.querySelector("#moodDescription"),
  note: document.querySelector("#moodNote")
};

const defaultsProfile002 = {
  bgColor: "#f4f1ea",
  frameBgColor: "#f4f1ea",
  textColor: "#8c8b83",
  miniColor: "#a9a77a",
  ...
  musicBgColor: "#f7f4ee",
  borderColor: "#c8c6a0",
  accentTextColor: "#7a7a63",

  dateText: "✦✧✦",
  searchText: "search",
  contactText: "Contact",
  miniCaption: "ordinary vampire",
  mainTitle: "Moodboard",

  leftImageOne: "https://i.pinimg.com/1200x/8c/ed/33/8ced33abf443ca2e26249f3db384b8db.jpg",
  leftImageOneX: 50,
  leftImageOneY: 50,
  leftImageTwo: "https://i.pinimg.com/736x/16/09/d6/1609d6488b04c9a03ceb1319b30399d0.jpg",
  leftImageTwoX: 50,
  leftImageTwoY: 50,

  centerImageOne: "https://i.pinimg.com/736x/9e/02/4b/9e024b2f0a3f3681696290bd7c6ffc6f.jpg",
  centerImageOneX: 50,
  centerImageOneY: 50,
  centerImageTwo: "https://i.pinimg.com/736x/2a/75/bd/2a75bd78ea9d22c967007e381601a2a7.jpg",
  centerImageTwoX: 50,
  centerImageTwoY: 50,
  ...
  rightImageOne: "https://i.pinimg.com/1200x/74/fa/7c/74fa7c72fb5006c92bab6d52bc224d13.jpg",
  rightImageOneX: 50,
  rightImageOneY: 50,
  rightImageTwo: "https://i.pinimg.com/736x/fd/d7/c0/fdd7c0ae86d73d9dfcd7e53907bc6d60.jpg",
  rightImageTwoX: 50,
  rightImageTwoY: 50,

  placeTitle: "Places",
  placeText: "Eiffel Tower, Paris",
  placeAvatar: "https://i.pinimg.com/1200x/06/96/4b/06964b4f18705984024f2e066c863688.jpg",
  placeAvatarX: 50,
  placeAvatarY: 50,

  placeImageOne: "https://i.pinimg.com/736x/12/37/56/123756c6286e0aaa6220dd07d61c8bd3.jpg",
  placeImageOneX: 50,
  placeImageOneY: 50,
  placeImageTwo: "https://i.pinimg.com/1200x/ac/35/ca/ac35ca94bb33823980458e12a54ef696.jpg",
  placeImageTwoX: 50,
  placeImageTwoY: 50,

  musicCover: "https://i.ytimg.com/vi/KQxlEmOwIOM/hq720.jpg",
  musicCoverX: 50,
  musicCoverY: 50,
  songTitle: "surreal",
  artistName: "limr",

  collageTop: "https://i.pinimg.com/736x/7a/b2/b4/7ab2b4c8e36a6da80a152dfae13e11a5.jpg",
  collageTopX: 50,
  collageTopY: 50,
  collageIris: "https://i.pinimg.com/736x/bb/8a/8b/bb8a8b0d99d81ddbc32dba1f0024c927.jpg",
  collageIrisX: 50,
  collageIrisY: 50,
  collagePortrait: "https://i.pinimg.com/1200x/82/9f/c1/829fc137926ba6e3e84e7f55cd53a5dc.jpg",
  collagePortraitX: 50,
  collagePortraitY: 50,
  collageFlower: "https://i.pinimg.com/736x/e7/8f/72/e78f7218323ae537264a77bf294b3f38.jpg",
  collageFlowerX: 50,
  collageFlowerY: 50,

  bottomPill: "LET ME TELL YOU",
  bottomSymbols: "── ☆☆☆☆☆",
  displayName: "Franklin",
  description: "In motion (Yeah, stay just like that)\nWe're flowin' (Only for me)\nJust like that, just you and me",
  note: "I wanna dive in deeper\nInto your ocean, I like you just like this"
};

const moodboardStylesheets = [
  "https://guindaeyo.github.io/deepdshop/ddshop-deepmood.css",
  "https://fonts.googleapis.com/css2?family=Bai+Jamjuree:wght@300;400;500;600&family=Playfair+Display:ital,wght@0,400;0,600;1,600&display=swap"
];

const moodboardFinalHead =
  `<link href="https://guindaeyo.github.io/deepdshop/ddshop-deepmood.css" rel="stylesheet"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Bai+Jamjuree:wght@300;400;500;600&family=Playfair+Display:ital,wght@0,400;0,600;1,600&display=swap" rel="stylesheet">`;

const moodboardEditorPreviewCss =
  `<style data-moodboard-preview-only>html,body{width:1100px!important;min-width:1100px!important;max-width:1100px!important;margin:0!important;padding:0!important;overflow:hidden!important}body{position:relative!important}.dds-preview-shell{width:1100px!important;min-width:1100px!important;max-width:1100px!important;height:auto!important;min-height:0!important;margin:0!important;padding:0!important;display:block!important;overflow:visible!important}.dds-preview-target{width:1100px!important;min-width:1100px!important;max-width:1100px!important;height:auto!important;min-height:0!important;margin:0!important;padding:0!important;position:relative!important;transform:none!important;overflow:visible!important}.deepmood-sb-mmmood{width:1100px!important;min-width:1100px!important;max-width:1100px!important;height:auto!important;min-height:0!important;margin:0!important;position:relative!important;overflow:visible!important}</style>`;

const moodboardCardPreviewCss =
  `<style data-moodboard-card-preview-only>html,body{width:1100px!important;min-width:1100px!important;max-width:1100px!important;margin:0!important;padding:0!important;overflow:hidden!important}body{position:relative!important}.dds-card-preview-shell{width:1100px!important;min-width:1100px!important;max-width:1100px!important;height:auto!important;min-height:0!important;margin:0!important;padding:0!important;display:block!important;overflow:visible!important}.dds-card-preview-target{width:1100px!important;min-width:1100px!important;max-width:1100px!important;height:auto!important;min-height:0!important;margin:0!important;padding:0!important;position:relative!important;transform:none!important;overflow:visible!important}.deepmood-sb-mmmood{width:1100px!important;min-width:1100px!important;max-width:1100px!important;height:auto!important;min-height:0!important;margin:0!important;position:relative!important;overflow:visible!important}</style>`;

function moodPosition(fieldX, fieldY, defaultX = 50, defaultY = 50) {
  return {
    x: normalizePosition(fieldX.value, defaultX),
    y: normalizePosition(fieldY.value, defaultY)
  };
}

function getMoodboardData() {
  const leftOne = moodPosition(fieldsProfile002.leftImageOneX, fieldsProfile002.leftImageOneY);
  const leftTwo = moodPosition(fieldsProfile002.leftImageTwoX, fieldsProfile002.leftImageTwoY);
  const centerOne = moodPosition(fieldsProfile002.centerImageOneX, fieldsProfile002.centerImageOneY);
  const centerTwo = moodPosition(fieldsProfile002.centerImageTwoX, fieldsProfile002.centerImageTwoY);
  const rightOne = moodPosition(fieldsProfile002.rightImageOneX, fieldsProfile002.rightImageOneY);
  const rightTwo = moodPosition(fieldsProfile002.rightImageTwoX, fieldsProfile002.rightImageTwoY);
  const placeAvatar = moodPosition(fieldsProfile002.placeAvatarX, fieldsProfile002.placeAvatarY);
  const placeOne = moodPosition(fieldsProfile002.placeImageOneX, fieldsProfile002.placeImageOneY);
  const placeTwo = moodPosition(fieldsProfile002.placeImageTwoX, fieldsProfile002.placeImageTwoY);
  const musicCover = moodPosition(fieldsProfile002.musicCoverX, fieldsProfile002.musicCoverY);
  const collageTop = moodPosition(fieldsProfile002.collageTopX, fieldsProfile002.collageTopY);
  const collageIris = moodPosition(fieldsProfile002.collageIrisX, fieldsProfile002.collageIrisY);
  const collagePortrait = moodPosition(fieldsProfile002.collagePortraitX, fieldsProfile002.collagePortraitY);
  const collageFlower = moodPosition(fieldsProfile002.collageFlowerX, fieldsProfile002.collageFlowerY);

  return {
    bgColor: normalizeColor(fieldsProfile002.bgColor.value, defaultsProfile002.bgColor),
    frameBgColor: normalizeColor(fieldsProfile002.frameBgColor.value, defaultsProfile002.frameBgColor),
    textColor: normalizeColor(fieldsProfile002.textColor.value, defaultsProfile002.textColor),
    miniColor: normalizeColor(fieldsProfile002.miniColor.value, defaultsProfile002.miniColor),
    titleColor: normalizeColor(fieldsProfile002.titleColor.value, defaultsProfile002.titleColor),
    tagTextColor: normalizeColor(fieldsProfile002.tagTextColor.value, defaultsProfile002.tagTextColor),
    oliveColor: normalizeColor(fieldsProfile002.oliveColor.value, defaultsProfile002.oliveColor),
    orangeColor: normalizeColor(fieldsProfile002.orangeColor.value, defaultsProfile002.orangeColor),
    musicBgColor: normalizeColor(fieldsProfile002.musicBgColor.value, defaultsProfile002.musicBgColor),
    borderColor: normalizeColor(fieldsProfile002.borderColor.value, defaultsProfile002.borderColor),
    accentTextColor: normalizeColor(fieldsProfile002.accentTextColor.value, defaultsProfile002.accentTextColor),

    dateText: fieldsProfile002.dateText.value,
    searchText: fieldsProfile002.searchText.value,
    contactText: fieldsProfile002.contactText.value,
    miniCaption: fieldsProfile002.miniCaption.value,
    mainTitle: fieldsProfile002.mainTitle.value,

    leftImageOne: fieldsProfile002.leftImageOne.value.trim(),
    leftImageOneX: leftOne.x,
    leftImageOneY: leftOne.y,
    leftImageTwo: fieldsProfile002.leftImageTwo.value.trim(),
    leftImageTwoX: leftTwo.x,
    leftImageTwoY: leftTwo.y,

    centerImageOne: fieldsProfile002.centerImageOne.value.trim(),
    centerImageOneX: centerOne.x,
    centerImageOneY: centerOne.y,
    centerImageTwo: fieldsProfile002.centerImageTwo.value.trim(),
    centerImageTwoX: centerTwo.x,
    centerImageTwoY: centerTwo.y,

    tagOne: fieldsProfile002.tagOne.value,
    tagTwo: fieldsProfile002.tagTwo.value,
    tagThree: fieldsProfile002.tagThree.value,
    tagFour: fieldsProfile002.tagFour.value,
    tagFive: fieldsProfile002.tagFive.value,
    tagSix: fieldsProfile002.tagSix.value,

    rightImageOne: fieldsProfile002.rightImageOne.value.trim(),
    rightImageOneX: rightOne.x,
    rightImageOneY: rightOne.y,
    rightImageTwo: fieldsProfile002.rightImageTwo.value.trim(),
    rightImageTwoX: rightTwo.x,
    rightImageTwoY: rightTwo.y,

    placeTitle: fieldsProfile002.placeTitle.value,
    placeText: fieldsProfile002.placeText.value,
    placeAvatar: fieldsProfile002.placeAvatar.value.trim(),
    placeAvatarX: placeAvatar.x,
    placeAvatarY: placeAvatar.y,

    placeImageOne: fieldsProfile002.placeImageOne.value.trim(),
    placeImageOneX: placeOne.x,
    placeImageOneY: placeOne.y,
    placeImageTwo: fieldsProfile002.placeImageTwo.value.trim(),
    placeImageTwoX: placeTwo.x,
    placeImageTwoY: placeTwo.y,

    musicCover: fieldsProfile002.musicCover.value.trim(),
    musicCoverX: musicCover.x,
    musicCoverY: musicCover.y,
    songTitle: fieldsProfile002.songTitle.value,
    artistName: fieldsProfile002.artistName.value,

    collageTop: fieldsProfile002.collageTop.value.trim(),
    collageTopX: collageTop.x,
    collageTopY: collageTop.y,
    collageIris: fieldsProfile002.collageIris.value.trim(),
    collageIrisX: collageIris.x,
    collageIrisY: collageIris.y,
    collagePortrait: fieldsProfile002.collagePortrait.value.trim(),
    collagePortraitX: collagePortrait.x,
    collagePortraitY: collagePortrait.y,
    collageFlower: fieldsProfile002.collageFlower.value.trim(),
    collageFlowerX: collageFlower.x,
    collageFlowerY: collageFlower.y,

    bottomPill: fieldsProfile002.bottomPill.value,
    bottomSymbols: fieldsProfile002.bottomSymbols.value,
    displayName: fieldsProfile002.displayName.value,
    description: fieldsProfile002.description.value,
    note: fieldsProfile002.note.value
  };
}

function moodImage(url, x, y, extraClass = "") {
  return `<img${extraClass ? ` class="${extraClass}"` : ""} src="${escapeHtml(url)}" style="object-fit:cover;object-position:${x}% ${y}%;">`;
}

function buildMoodboardMarkup(data) {
  return `<div class="deepmood-sb-mmmood" style="--bgmood:${data.bgColor};--mmbg:${data.frameBgColor};--mmtxtx:${data.textColor};--mmmiticapt:${data.miniColor};--mm01mood:${data.titleColor};--mmtxttag:${data.tagTextColor};--olive:${data.oliveColor};--ornn:${data.orangeColor};--mmcardmu:${data.musicBgColor};--mmborder:${data.borderColor};--conla1:${data.accentTextColor};"><div class="deepmood-sb-mood"><div class="deepmood-sb-top"><div class="deepmood-sb-date">${formatEmojiOrSymbol(data.dateText)}</div><div class="deepmood-sb-right"><div class="deepmood-sb-search">${escapeHtml(data.searchText)}</div><div class="deepmood-sb-contact">${escapeHtml(data.contactText)}</div></div></div><div class="deepmood-sb-title"><div class="deepmood-mini">${escapeHtml(data.miniCaption)}</div><h1>${escapeHtml(data.mainTitle)}</h1></div><div class="deepmood-sb-grid"><div class="deepmood-sb-left"><div class="deepmood-sb-img deepmood-img-l1">${moodImage(data.leftImageOne,data.leftImageOneX,data.leftImageOneY)}</div><div class="deepmood-sb-img deepmood-img-l2">${moodImage(data.leftImageTwo,data.leftImageTwoX,data.leftImageTwoY)}</div></div><div class="deepmood-sb-center"><div class="deepmood-sb-img deepmood-img-c1">${moodImage(data.centerImageOne,data.centerImageOneX,data.centerImageOneY)}</div><div class="deepmood-sb-img deepmood-img-c2">${moodImage(data.centerImageTwo,data.centerImageTwoX,data.centerImageTwoY)}</div><div class="deepmood-sb-tags"><div class="deepmood-sb-tag">${escapeHtml(data.tagOne)}</div><div class="deepmood-sb-tag alt">${escapeHtml(data.tagTwo)}</div><div class="deepmood-sb-tag">${escapeHtml(data.tagThree)}</div><div class="deepmood-sb-tag alt">${escapeHtml(data.tagFour)}</div><div class="deepmood-sb-tag">${escapeHtml(data.tagFive)}</div><div class="deepmood-sb-tag alt">${escapeHtml(data.tagSix)}</div></div></div><div class="deepmood-sb-rightcol"><div class="deepmood-sb-img deepmood-img-r1">${moodImage(data.rightImageOne,data.rightImageOneX,data.rightImageOneY)}</div><div class="deepmood-sb-img deepmood-img-z1">${moodImage(data.rightImageTwo,data.rightImageTwoX,data.rightImageTwoY)}</div><div class="deepmood-sb-places"><div class="deepmood-t"><b>${escapeHtml(data.placeTitle)}</b><span>${escapeHtml(data.placeText)}</span></div>${moodImage(data.placeAvatar,data.placeAvatarX,data.placeAvatarY,"deepmood-pav")}</div><div class="deepmood-sb-rightimgs"><div class="deepmood-sb-img deepmood-img-r2">${moodImage(data.placeImageOne,data.placeImageOneX,data.placeImageOneY)}</div><div class="deepmood-sb-img deepmood-img-r3">${moodImage(data.placeImageTwo,data.placeImageTwoX,data.placeImageTwoY)}</div></div></div></div><div class="deepmood-sb-bottom"><div class="deepmood-sb-player">${moodImage(data.musicCover,data.musicCoverX,data.musicCoverY,"deepmood-cover")}<div class="deepmood-meta"><b>${escapeHtml(data.songTitle)}</b><span>${escapeHtml(data.artistName)}</span></div></div></div></div><div class="deepmood-oceaneyes-post"><div class="deepmood-oceaneyes-collage"><div class="deepmood-oe-frame deepmood-oe-topwide"><div class="deepmood-oe-inner">${moodImage(data.collageTop,data.collageTopX,data.collageTopY)}</div></div><div class="deepmood-oe-frame deepmood-oe-iris"><div class="deepmood-oe-inner">${moodImage(data.collageIris,data.collageIrisX,data.collageIrisY)}</div></div><div class="deepmood-oe-frame deepmood-oe-portrait"><div class="deepmood-oe-inner">${moodImage(data.collagePortrait,data.collagePortraitX,data.collagePortraitY)}</div></div><div class="deepmood-oe-frame deepmood-oe-flower"><div class="deepmood-oe-inner">${moodImage(data.collageFlower,data.collageFlowerX,data.collageFlowerY)}</div></div></div></div><div class="deepmood-igbar"><div class="deepmood-igbar-top"><div class="deepmood-igbar-pill">${escapeHtml(data.bottomPill)}</div><div class="deepmood-igbar-enen">${escapeHtml(data.bottomSymbols)}</div></div><div class="deepmood-igbar-main"><div class="deepmood-igbar-name">${escapeHtml(data.displayName)}</div><div class="deepmood-igbar-desc">${textWithBreaks(data.description)}</div><div class="deepmood-igbar-right"><div class="deepmood-igbar-note">${textWithBreaks(data.note)}</div></div></div></div></div>
${fdreviewCreditMarkup}`;
}

function buildMoodboardFinalCode(data) {
  return `${moodboardFinalHead}
${buildCreditStyleTag()}
${buildMoodboardMarkup(data)}`;
}

function updateMoodboard() {
  if (!generatedMoodboardCode) {
    return;
  }
  ...
  generatedMoodboardCode.value =
    buildMoodboardFinalCode(data);

  const editorMarkup =
    moodboardEditorPreviewCss +
    buildMoodboardMarkup(data);

  const cardMarkup =
    moodboardCardPreviewCss +
    buildMoodboardMarkup(data);
...
/* ==================================================
   PROFILE CODE003 — FORTYONEWINKS
================================================== */

const fortyOnePreview =
  document.querySelector(
    "#fortyOnePreview"
  );

const profileCardPreview003 =
  document.querySelector(
    "#profileCardPreview003"
  );

const generatedFortyOneCode =
  document.querySelector(
    "#generatedFortyOneCode"
  );

const fieldsProfile003 = {
  cardBgColor:
    document.querySelector(
      "#fortyCardBgColor"
    ),
  ...
  backgroundImage:
    document.querySelector(
      "#fortyBackgroundImage"
    ),

  backgroundImageX:
    document.querySelector(
      "#fortyBackgroundImageX"
    ),

  backgroundImageY:
    document.querySelector(
      "#fortyBackgroundImageY"
    ),
...
const defaultsProfile003 = {
  ...
  symbolColor: "#c5e1a5",
  symbolTextColor: "#bdbdbd",
  boxOneColor: "#c5d6ba",
  boxTwoColor: "#f2e9d3",
  boxThreeColor: "#d6c3ce",

  backgroundImage:
    "https://i.pinimg.com/1200x/98/63/d9/9863d94dbcb62d9f3248e6163cb6f90a.jpg",
  backgroundImageX: 50,
  backgroundImageY: 50,

  whisperText: "our love has died",
  symbolOne: "*",
  symbolTwo: "☘",
  species: "Vampire",

  avatarImage:
    "https://i.pinimg.com/736x/ab/32/2f/ab322f9a5d04f4cecb2dbf415fa9f300.jpg",
  avatarImageX: 50,
  avatarImageY: 50,
  ...
  lastName: "Bloodworth",
  profileUrl:
    "https://roleplayth.com/showthread.php?tid=3532",
  profileButtonText: "Profile",

  bodyText:
    "Foolishly, I gave my soul to you.\nI let you... infest.\nBelieving your lies. And\nyou don't remember.\n\nHelplessly, I fell so deep\nI was so naive to let you in\nWhy did I let you in\nto my heart?\n\nAnd you don't remember.\nEvery time you told me,\nAnything that you told me\nYou were mine forever",

  footerTitle: "LET ME TELL YOU",
  footerSubtitle:
    "Just like that, just you and me"
};

const fortyOneStylesheets = [
  "https://guindaeyo.github.io/deepdshop/ddshop-FortyOneWinks.css",
  "https://fonts.googleapis.com/css2?family=Bai+Jamjuree:wght@300;400;500;600&family=Cormorant+Garamond:ital,wght@0,400;0,600;1,600&display=swap"
];

const fortyOneFinalHead =
  `<link href="https://guindaeyo.github.io/deepdshop/ddshop-FortyOneWinks.css" rel="stylesheet"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="https://fonts.googleapis.com/css2?family=Bai+Jamjuree:wght@300;400;500;600&family=Cormorant+Garamond:ital,wght@0,400;0,600;1,600&display=swap" rel="stylesheet">`;

const fortyOneEditorPreviewCss =
  `<style data-fortyone-preview-only>html,body{width:1000px!important;min-width:1000px!important;max-width:1000px!important;height:1700px!important;min-height:1700px!important;max-height:1700px!important;margin:0!important;padding:0!important;overflow:hidden!important}body{position:relative!important}.dds-preview-shell,.dds-preview-target{width:1000px!important;min-width:1000px!important;max-width:1000px!important;height:1700px!important;min-height:1700px!important;max-height:1700px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important;transform:none!important}.dds-fortyone-preview-content{width:1000px!important;min-width:1000px!important;max-width:1000px!important;height:1700px!important;min-height:1700px!important;max-height:1700px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important}.dds-fortyone-preview-content>.ddpshop-ywmf{position:absolute!important;top:50%!important;left:50%!important;right:auto!important;bottom:auto!important;margin:0!important;transform:translate(-50%,-50%)!important;transform-origin:center center!important}.dds-fortyone-preview-content>.fdreview-credit{position:absolute!important;left:0!important;right:0!important;bottom:3px!important;width:100%!important;transform:none!important}</style>`;

const fortyOneCardPreviewCss =
  `<style data-fortyone-card-preview-only>html,body{width:1000px!important;min-width:1000px!important;max-width:1000px!important;height:1700px!important;min-height:1700px!important;max-height:1700px!important;margin:0!important;padding:0!important;overflow:hidden!important}body{position:relative!important}.dds-card-preview-shell,.dds-card-preview-target{width:1000px!important;min-width:1000px!important;max-width:1000px!important;height:1700px!important;min-height:1700px!important;max-height:1700px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important;transform:none!important}.dds-fortyone-preview-content{width:1000px!important;min-width:1000px!important;max-width:1000px!important;height:1700px!important;min-height:1700px!important;max-height:1700px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important}.dds-fortyone-preview-content>.ddpshop-ywmf{position:absolute!important;top:50%!important;left:50%!important;right:auto!important;bottom:auto!important;margin:0!important;transform:translate(-50%,-50%)!important;transform-origin:center center!important}.dds-fortyone-preview-content>.fdreview-credit{position:absolute!important;left:0!important;right:0!important;bottom:3px!important;width:100%!important;transform:none!important}</style>`;

function getFortyOneData() {
  return {
    cardBgColor:
      normalizeColor(
        fieldsProfile003.cardBgColor.value,
        defaultsProfile003.cardBgColor
      ),

    textVColor:
      normalizeColor(
        fieldsProfile003.textVColor.value,
        defaultsProfile003.textVColor
      ),

    mainTextColor:
      normalizeColor(
        fieldsProfile003.mainTextColor.value,
        defaultsProfile003.mainTextColor
      ),

    buttonBgColor:
      normalizeColor(
        fieldsProfile003.buttonBgColor.value,
        defaultsProfile003.buttonBgColor
      ),

    buttonTextColor:
      normalizeColor(
        fieldsProfile003.buttonTextColor.value,
        defaultsProfile003.buttonTextColor
      ),

    symbolColor:
      normalizeColor(
        fieldsProfile003.symbolColor.value,
        defaultsProfile003.symbolColor
      ),

    symbolTextColor:
      normalizeColor(
        fieldsProfile003.symbolTextColor.value,
        defaultsProfile003.symbolTextColor
      ),

    boxOneColor:
      normalizeColor(
        fieldsProfile003.boxOneColor.value,
        defaultsProfile003.boxOneColor
      ),

    boxTwoColor:
      normalizeColor(
        fieldsProfile003.boxTwoColor.value,
        defaultsProfile003.boxTwoColor
      ),

    boxThreeColor:
      normalizeColor(
        fieldsProfile003.boxThreeColor.value,
        defaultsProfile003.boxThreeColor
      ),

    backgroundImage:
      fieldsProfile003.backgroundImage.value.trim(),

    backgroundImageX:
      normalizePosition(
        fieldsProfile003.backgroundImageX.value,
        defaultsProfile003.backgroundImageX
      ),

    backgroundImageY:
      normalizePosition(
        fieldsProfile003.backgroundImageY.value,
        defaultsProfile003.backgroundImageY
      ),

    whisperText:
      fieldsProfile003.whisperText.value,

    symbolOne:
      fieldsProfile003.symbolOne.value,

    symbolTwo:
      fieldsProfile003.symbolTwo.value,

    species:
      fieldsProfile003.species.value,

    avatarImage:
      fieldsProfile003.avatarImage.value.trim(),

    avatarImageX:
      normalizePosition(
        fieldsProfile003.avatarImageX.value,
        defaultsProfile003.avatarImageX
      ),

    avatarImageY:
      normalizePosition(
        fieldsProfile003.avatarImageY.value,
        defaultsProfile003.avatarImageY
      ),

    firstName:
      fieldsProfile003.firstName.value,

    lastName:
      fieldsProfile003.lastName.value,

    profileUrl:
      fieldsProfile003.profileUrl.value.trim(),

    profileButtonText:
      fieldsProfile003.profileButtonText.value,

    bodyText:
      fieldsProfile003.bodyText.value,

    footerTitle:
      fieldsProfile003.footerTitle.value,

    footerSubtitle:
      fieldsProfile003.footerSubtitle.value
  };
}
...
function buildFortyOneFinalCode(data) {
  return `${fortyOneFinalHead}
${buildCreditStyleTag()}
${buildFortyOneMarkup(data)}`;
}

function updateFortyOne() {
  if (!generatedFortyOneCode) {
    return;
  }
  ...
  generatedFortyOneCode.value =
    buildFortyOneFinalCode(data);

  const editorMarkup =
    fortyOneEditorPreviewCss +
    `<div class="dds-fortyone-preview-content">${buildFortyOneMarkup(data)}</div>`;

  const cardMarkup =
    fortyOneCardPreviewCss +
    `<div class="dds-fortyone-preview-content">${buildFortyOneMarkup(data)}</div>`;
...
/* ==================================================
   PROFILE CODE004 — NOTHIN’ ’BOUT ME
================================================== */
...
const profileCardPreview004 =
  document.querySelector(
    "#profileCardPreview004"
  );

const generatedNothinCode =
  document.querySelector(
    "#generatedNothinCode"
  );

const fieldsProfile004 = {
  outerBgColor:
    document.querySelector(
      "#nothinOuterBgColor"
    ),
...
const defaultsProfile004 = {
  ...
  mainImage:
    "https://i.pinimg.com/736x/6e/52/c3/6e52c33334a679b84a99d03cafbd4d3b.jpg",
  mainImageX: 50,
  mainImageY: 50,

  fieldImage:
    "https://i.pinimg.com/736x/56/95/fb/5695fbd274e2b0c173ea730606a8ccdf.jpg",
  fieldImageX: 50,
  fieldImageY: 50,

  fourImage:
    "https://i.pinimg.com/1200x/bf/dd/89/bfdd890da4d3f4b8f4ef896cf67604ad.jpg",
  fourImageX: 50,
  fourImageY: 50,
  ...
  mainTag: "BEST NEWS",
  mainSymbol: "♥",
  seasonTextOne: "stubborn",
  seasonTextTwo: "naughty",
  productText: "PRODUCT",
  qualityText: "QUALITY",
  videoText: "VIDEO",
  bottomSymbol: "✦"
...
const nothinStylesheets = [
  "https://guindaeyo.github.io/deepdshop/ddsh-nothinboutme.css"
];

const nothinFinalHead =
  `<link href="https://guindaeyo.github.io/deepdshop/ddsh-nothinboutme.css" rel="stylesheet">`;

const nothinEditorPreviewCss =
  `<style data-nothin-preview-only>html,body{width:1200px!important;min-width:1200px!important;max-width:1200px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;overflow:hidden!important}body{position:relative!important}.dds-preview-shell,.dds-preview-target{width:1200px!important;min-width:1200px!important;max-width:1200px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important;transform:none!important}.dds-nothin-preview-content{width:1200px!important;min-width:1200px!important;max-width:1200px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important}.dds-nothin-preview-content>.boutmeddsh{position:absolute!important;top:50%!important;left:50%!important;right:auto!important;bottom:auto!important;margin:0!important;transform:translate(-50%,-50%)!important;transform-origin:center center!important}.dds-nothin-preview-content>.fdreview-credit{position:absolute!important;left:0!important;right:0!important;bottom:3px!important;width:100%!important;transform:none!important}</style>`;

const nothinCardPreviewCss =
  `<style data-nothin-card-preview-only>html,body{width:1200px!important;min-width:1200px!important;max-width:1200px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;overflow:hidden!important}body{position:relative!important}.dds-card-preview-shell,.dds-card-preview-target{width:1200px!important;min-width:1200px!important;max-width:1200px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important;transform:none!important}.dds-nothin-preview-content{width:1200px!important;min-width:1200px!important;max-width:1200px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important}.dds-nothin-preview-content>.boutmeddsh{position:absolute!important;top:50%!important;left:50%!important;right:auto!important;bottom:auto!important;margin:0!important;transform:translate(-50%,-50%)!important;transform-origin:center center!important}.dds-nothin-preview-content>.fdreview-credit{position:absolute!important;left:0!important;right:0!important;bottom:3px!important;width:100%!important;transform:none!important}</style>`;

function getNothinData() {
  return {
    outerBgColor:
      normalizeColor(
        fieldsProfile004.outerBgColor.value,
        defaultsProfile004.outerBgColor
      ),

    frameBgColor:
      normalizeColor(
        fieldsProfile004.frameBgColor.value,
        defaultsProfile004.frameBgColor
      ),

    dotColor:
      normalizeColor(
        fieldsProfile004.dotColor.value,
        defaultsProfile004.dotColor
      ),

    mainColor:
      normalizeColor(
        fieldsProfile004.mainColor.value,
        defaultsProfile004.mainColor
      ),

    textColor:
      normalizeColor(
        fieldsProfile004.textColor.value,
        defaultsProfile004.textColor
      ),

    borderColor:
      normalizeColor(
        fieldsProfile004.borderColor.value,
        defaultsProfile004.borderColor
      ),

    mainImage:
      fieldsProfile004.mainImage.value.trim(),

    mainImageX:
      normalizePosition(
        fieldsProfile004.mainImageX.value,
        defaultsProfile004.mainImageX
      ),

    mainImageY:
      normalizePosition(
        fieldsProfile004.mainImageY.value,
        defaultsProfile004.mainImageY
      ),

    fieldImage:
      fieldsProfile004.fieldImage.value.trim(),

    fieldImageX:
      normalizePosition(
        fieldsProfile004.fieldImageX.value,
        defaultsProfile004.fieldImageX
      ),

    fieldImageY:
      normalizePosition(
        fieldsProfile004.fieldImageY.value,
        defaultsProfile004.fieldImageY
      ),

    fourImage:
      fieldsProfile004.fourImage.value.trim(),

    fourImageX:
      normalizePosition(
        fieldsProfile004.fourImageX.value,
        defaultsProfile004.fourImageX
      ),

    fourImageY:
      normalizePosition(
        fieldsProfile004.fourImageY.value,
        defaultsProfile004.fourImageY
      ),

    species:
      fieldsProfile004.species.value,

    displayName:
      fieldsProfile004.displayName.value,

    birthday:
      fieldsProfile004.birthday.value,

    mainTag:
      fieldsProfile004.mainTag.value,

    mainSymbol:
      fieldsProfile004.mainSymbol.value,

    seasonTextOne:
      fieldsProfile004.seasonTextOne.value,

    seasonTextTwo:
      fieldsProfile004.seasonTextTwo.value,

    productText:
      fieldsProfile004.productText.value,

    qualityText:
      fieldsProfile004.qualityText.value,

    videoText:
      fieldsProfile004.videoText.value,

    bottomSymbol:
      fieldsProfile004.bottomSymbol.value
  };
}
...
function buildNothinFinalCode(data) {
  return `${nothinFinalHead}
${buildCreditStyleTag()}
${buildNothinMarkup(data)}`;
}

function updateNothinBoutMe() {
  if (!generatedNothinCode) {
    return;
  }
  ...
  generatedNothinCode.value =
    buildNothinFinalCode(data);

  const editorMarkup =
    nothinEditorPreviewCss +
    `<div class="dds-nothin-preview-content">${buildNothinMarkup(data)}</div>`;

  const cardMarkup =
    nothinCardPreviewCss +
    `<div class="dds-nothin-preview-content">${buildNothinMarkup(data)}</div>`;
...
/* ==================================================
   COMMISSION 001 — LANDON A. RUTHERFORD
================================================== */

const commissionCardPreview001 =
  document.querySelector(
    "#commissionCardPreview001"
  );

const commissionPreview001 =
  document.querySelector(
    "#commissionPreview001"
  );

const commissionStylesheets001 = [
  "https://guindaeyo.github.io/css/foodierv-land.css"
];

const commissionPreviewCss001 =
  `<style data-commission-preview-only>html,body{width:1500px!important;min-width:1500px!important;max-width:1500px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;overflow:hidden!important;background:#242424!important}body{position:relative!important}.dds-preview-shell,.dds-preview-target,.dds-card-preview-shell,.dds-card-preview-target{width:1500px!important;min-width:1500px!important;max-width:1500px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important;transform:none!important}.dds-commission-preview-content{width:1500px!important;min-width:1500px!important;max-width:1500px!important;height:1000px!important;min-height:1000px!important;max-height:1000px!important;margin:0!important;padding:0!important;display:flex!important;align-items:center!important;justify-content:center!important;position:relative!important;overflow:hidden!important}.dds-commission-post-column{width:734px!important;min-width:734px!important;max-width:734px!important;height:800px!important;min-height:800px!important;max-height:800px!important;margin:0!important;padding:0!important;position:relative!important;overflow:hidden!important;background:#fff!important}.dds-commission-post-column>.fdreview-wrap{width:734px!important;min-width:734px!important;max-width:734px!important;height:auto!important;min-height:0!important;max-height:none!important;margin:0!important;position:relative!important;overflow:hidden!important}</style>`;
...
function updateCommissionWork001() {
  const markup =
    commissionPreviewCss001 +
    `<div class="dds-commission-preview-content"><div class="dds-commission-post-column">${commissionMarkup001}</div></div>`;

  if (commissionCardPreview001) {
    queuePreviewDocument(
      commissionCardPreview001,
      buildCardPreviewDocument(
        commissionStylesheets001,
        markup
      ),
      resizeCommissionFixedPreview
    );
...
const resetMoodboardZoomButton =
  document.querySelector(
    "#resetMoodboardZoom"
  );

if (resetMoodboardZoomButton) {
  resetMoodboardZoomButton
    .addEventListener(
      "click",
      () => {
        setMoodboardZoom(1);
...
const resetFortyOneZoomButton =
  document.querySelector(
    "#resetFortyOneZoom"
  );

if (resetFortyOneZoomButton) {
  resetFortyOneZoomButton
    .addEventListener(
      "click",
      () => {
        setFortyOneZoom(1);
...
const resetNothinZoomButton =
  document.querySelector(
    "#resetNothinZoom"
  );

if (resetNothinZoomButton) {
  resetNothinZoomButton
    .addEventListener(
      "click",
      () => {
        setNothinZoom(1);
...
Object.values(fieldsProfile002).forEach((field) => {
  if (!field) {
    return;
  }
...
Object.values(fieldsProfile003).forEach((field) => {
  if (!field) {
    return;
  }
...
Object.values(fieldsProfile004).forEach((field) => {
  if (!field) {
    return;
  }
...
bindColorPairs(
  [
    ["moodBgColor", "moodBgColorPicker"],
    ["moodFrameBgColor", "moodFrameBgColorPicker"],
    ["moodTextColor", "moodTextColorPicker"],
    ["moodMiniColor", "moodMiniColorPicker"],
    ["moodTitleColor", "moodTitleColorPicker"],
    ["moodTagTextColor", "moodTagTextColorPicker"],
    ["moodOliveColor", "moodOliveColorPicker"],
    ["moodOrangeColor", "moodOrangeColorPicker"],
    ["moodMusicBgColor", "moodMusicBgColorPicker"],
    ["moodBorderColor", "moodBorderColorPicker"],
    ["moodAccentTextColor", "moodAccentTextColorPicker"]
  ],
  updateMoodboard
);
...
bindColorPairs(
  [
    ["fortyCardBgColor", "fortyCardBgColorPicker"],
    ["fortyTextVColor", "fortyTextVColorPicker"],
    ["fortyMainTextColor", "fortyMainTextColorPicker"],
    ["fortyButtonBgColor", "fortyButtonBgColorPicker"],
    ["fortyButtonTextColor", "fortyButtonTextColorPicker"],
    ["fortySymbolColor", "fortySymbolColorPicker"],
    ["fortySymbolTextColor", "fortySymbolTextColorPicker"],
    ["fortyBoxOneColor", "fortyBoxOneColorPicker"],
    ["fortyBoxTwoColor", "fortyBoxTwoColorPicker"],
    ["fortyBoxThreeColor", "fortyBoxThreeColorPicker"]
  ],
  updateFortyOne
);
...
bindColorPairs(
  [
    ["nothinOuterBgColor", "nothinOuterBgColorPicker"],
    ["nothinFrameBgColor", "nothinFrameBgColorPicker"],
    ["nothinDotColor", "nothinDotColorPicker"],
    ["nothinMainColor", "nothinMainColorPicker"],
    ["nothinTextColor", "nothinTextColorPicker"],
    ["nothinBorderColor", "nothinBorderColorPicker"]
  ],
  updateNothinBoutMe
);
...
const copyProfileButton002 =
  document.querySelector(
    "#copyGeneratedMoodboardCode"
  );

if (copyProfileButton002) {
  copyProfileButton002.addEventListener(
    "click",
    async () => {
      updateMoodboard();
      ...
      showToast(
        "คัดลอก PROFILE CODE002 เรียบร้อยแล้ว"
      );
    }
  );
}
...
const copyProfileButton003 =
  document.querySelector(
    "#copyGeneratedFortyOneCode"
  );

if (copyProfileButton003) {
  copyProfileButton003.addEventListener(
    "click",
    async () => {
      updateFortyOne();
      ...
      showToast(
        "คัดลอก PROFILE CODE003 เรียบร้อยแล้ว"
      );
    }
  );
}
...
const copyProfileButton004 =
  document.querySelector(
    "#copyGeneratedNothinCode"
  );

if (copyProfileButton004) {
  copyProfileButton004.addEventListener(
    "click",
    async () => {
      updateNothinBoutMe();
      ...
      showToast(
        "คัดลอก PROFILE CODE004 เรียบร้อยแล้ว"
      );
    }
  );
}
...
const resetProfileButton002 =
  document.querySelector(
    "#resetMoodboard"
  );

if (resetProfileButton002) {
  resetProfileButton002.addEventListener(
    "click",
    () => {
      Object.entries(
        defaultsProfile002
      ...
      showToast(
        "คืนค่า PROFILE CODE002 เรียบร้อยแล้ว"
      );
    }
  );
}
...
const resetProfileButton003 =
  document.querySelector(
    "#resetFortyOne"
  );

if (resetProfileButton003) {
  resetProfileButton003.addEventListener(
    "click",
    () => {
      Object.entries(
        defaultsProfile003
      ...
      showToast(
        "คืนค่า PROFILE CODE003 เรียบร้อยแล้ว"
      );
    }
  );
}
...
const resetProfileButton004 =
  document.querySelector(
    "#resetNothin"
  );

if (resetProfileButton004) {
  resetProfileButton004.addEventListener(
    "click",
    () => {
      Object.entries(
        defaultsProfile004
      ...
      showToast(
        "คืนค่า PROFILE CODE004 เรียบร้อยแล้ว"
      );
    }
  );
}
