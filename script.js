...
function buildPreviewOnlyStyle(css) {
  return `<style data-dds-preview-only>
${css}
</style>`;
}
...
function resizeEditorPreview(iframe) {
  ...
  try {
    ...
    const naturalWidth =
      Math.max(
        target.scrollWidth,
        previewCodeRoot?.scrollWidth || 0
      );

    const naturalHeight =
      Math.max(
        target.scrollHeight,
        previewCodeRoot?.scrollHeight || 0
      );
    ...
    const scale =
      baseScale * previewZoom;

    target.style.transform =
      `scale(${scale})`;

    const viewportMode =
      iframe.dataset.previewMode ===
      "viewport";
    ...
    if (viewportMode) {
      ...
      if (previewDocument.body) {
        previewDocument.body.style.overflow =
          "auto";
      }
...
function updateComma() {
  ...
  const commaPreviewMarkup =
    buildPreviewOnlyStyle(
      commaPreviewOnlyCss
    ) +
    buildCommaMarkup(data);
...
function updateNewRules() {
  ...
  const newRulesPreviewMarkup =
    buildPreviewOnlyStyle(
      newRulesPreviewOnlyCss
    ) +
    buildNewRulesMarkup(data);

  if (newRulesPreview) {
    newRulesPreview.srcdoc =
      buildEditorPreviewDocument(
        newRulesStylesheets,
        newRulesPreviewMarkup
      );
  }

  if (roleplayCardPreview006) {
    roleplayCardPreview006.srcdoc =
      buildCardPreviewDocument(
        newRulesStylesheets,
        newRulesPreviewMarkup
      );
...
function updatePreviewZoomOutput(iframe) {
  if (!iframe) {
    return;
  }
  ...
  const output =
    document.querySelector(
      `[data-preview-zoom-output="${iframe.id}"]`
    );
...
function refreshInteractivePreview(iframe) {
  if (!iframe) {
    return;
  }

  updatePreviewZoomOutput(iframe);
  resizeEditorPreview(iframe);
}

document
  .querySelectorAll(
    "[data-preview-zoom-target]"
  )
  .forEach((button) => {
    button.addEventListener(
      "click",
      () => {
        const iframe =
          document.getElementById(
            button.dataset.previewZoomTarget
          );
        ...
        const step =
          Number(
            button.dataset.previewZoomStep || 0
          ) || 0;
        ...
        iframe.dataset.previewZoom =
          nextZoom.toFixed(2);

        refreshInteractivePreview(iframe);
      }
    );
  });
  ...
  .forEach((button) => {
    button.addEventListener(
      ...
      () => {
        const iframe =
          document.getElementById(
            button.dataset.previewZoomReset
          );
