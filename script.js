...
(() => {
  ...
  function buildPreviewDocument(markup, mode) {
    if (mode === "card") {
       ...
       */
      return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link href="${CSS_URL}" rel="stylesheet"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="${FONT_URL}" rel="stylesheet"><style>html,body{margin:0;width:1300px;min-width:1300px;max-width:1300px;height:920px;min-height:920px;max-height:920px;background:#242424;overflow:hidden}.dds-music-preview-shell{width:1300px;height:920px;display:flex;align-items:center;justify-content:center;overflow:hidden}.dds-music-preview-target{position:static;width:max-content;max-width:none;flex:0 0 auto;transform:none!important;transform-origin:center center}.dds-music-preview-target>.ddsh-revmus-wrap{margin:0!important}</style></head><body data-preview-mode="card"><div class="dds-music-preview-shell"><div class="dds-preview-target dds-music-preview-target">${markup}</div></div><script>(()=>{const ready=()=>{window.frameElement?.classList.add('dds-preview-ready');window.frameElement?.classList.remove('dds-preview-loading')};window.addEventListener('load',ready,{once:true});document.fonts?.ready?.then(ready);ready()})();<\/script></body></html>`;
    }

    return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link href="${CSS_URL}" rel="stylesheet"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="${FONT_URL}" rel="stylesheet"><style>html,body{margin:0;width:100%;height:100%;min-height:100%;background:#242424;overflow:hidden}.dds-music-preview-shell{position:relative;width:100%;height:100%;min-height:100%;overflow:hidden}.dds-music-preview-target{position:absolute;left:50%;width:max-content;will-change:transform}.dds-music-preview-target>.ddsh-revmus-wrap{margin:0!important}</style></head><body data-preview-mode="editor"><div class="dds-music-preview-shell"><div class="dds-preview-target dds-music-preview-target">${markup}</div></div><script>(()=>{const shell=document.querySelector('.dds-music-preview-shell');const target=document.querySelector('.dds-music-preview-target');const wrap=document.querySelector('.ddsh-revmus-wrap');let raf=0;function fit(){cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{if(!target||!wrap||!shell)return;const vw=Math.max(shell.clientWidth,document.documentElement.clientWidth,1);target.style.top='14px';target.style.left='50%';target.style.transformOrigin='top center';target.style.transform='translateX(-50%) scale(1)';const rect=wrap.getBoundingClientRect();const w=Math.max(wrap.scrollWidth,wrap.offsetWidth,rect.width,1);const h=Math.max(wrap.scrollHeight,wrap.offsetHeight,rect.height,1);const scale=Math.max(Math.min((vw-28)/w,1),.08);target.style.transform='translateX(-50%) scale('+scale+')';const scaledHeight=Math.ceil(h*scale+28);shell.style.height=scaledHeight+'px';document.body.style.height=scaledHeight+'px';if(window.frameElement){window.frameElement.style.height=Math.max(720,scaledHeight)+'px';window.frameElement.classList.add('dds-preview-ready');window.frameElement.classList.remove('dds-preview-loading')}})}window.addEventListener('load',fit);window.addEventListener('resize',fit);document.fonts?.ready?.then(fit);if(window.ResizeObserver&&wrap)new ResizeObserver(fit).observe(wrap);setTimeout(fit,80);setTimeout(fit,350);window.__ddsFitMusic=fit;fit()})();<\/script></body></html>`;
  }
