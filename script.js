...
const roleplayCardPreview001 = document.querySelector("#roleplayCardPreview001");
...
function buildCardPreviewDocument(data) {
  const markup = buildPageOfOneMarkup(data);
...
function resizePageOfOnePreview() {
  if (!pageOfOnePreview) return;

  try {
    const previewDocument = pageOfOnePreview.contentDocument;
    const wrapper = previewDocument?.querySelector(".pageof-wrapper");
    const shell = previewDocument?.querySelector(".dds-preview-shell");
    ...
    const availableWidth = Math.max(
      300,
      pageOfOnePreview.clientWidth - 44
    );
    ...
    pageOfOnePreview.style.height =
      `${Math.ceil(naturalHeight * scale) + 58}px`;
  } catch (error) {
    pageOfOnePreview.style.height = "1000px";
  }
}
...
function updatePageOfOne() {
  ...
  if (pageOfOnePreview) {
    pageOfOnePreview.srcdoc =
      buildPreviewDocument(data);
  }

  if (roleplayCardPreview001) {
    roleplayCardPreview001.srcdoc =
      buildCardPreviewDocument(data);
  }
}

if (pageOfOnePreview) {
  pageOfOnePreview.addEventListener("load", () => {
    requestAnimationFrame(resizePageOfOnePreview);

    setTimeout(
      resizePageOfOnePreview,
      180
    );
