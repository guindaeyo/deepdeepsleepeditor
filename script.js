...
(() => {
  ...
  function buildPreviewDocument(markup, mode) {
    return `<!DOCTYPE html><html lang="th"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><link href="${CSS_URL}" rel="stylesheet"><link rel="preconnect" href="https://fonts.googleapis.com"><link rel="preconnect" href="https://fonts.gstatic.com" crossorigin><link href="${FONT_URL}" rel="stylesheet"><style>html,body{margin:0;width:100%;height:100%;min-height:100%;background:#242424;overflow:hidden}.dds-music-preview-shell{position:relative;width:100%;height:100%;min-height:100%;overflow:hidden}.dds-music-preview-target{position:absolute;left:50%;width:max-content;will-change:transform}.dds-music-preview-target>.ddsh-revmus-wrap{margin:0!important}</style></head><body data-preview-mode="${mode}"><div class="dds-music-preview-shell"><div class="dds-preview-target dds-music-preview-target">${markup}</div></div><script>(()=>{const mode=document.body.dataset.previewMode;const isCard=mode==='card';const shell=document.querySelector('.dds-music-preview-shell');const target=document.querySelector('.dds-music-preview-target');const wrap=document.querySelector('.ddsh-revmus-wrap');let raf=0;function fit(){cancelAnimationFrame(raf);raf=requestAnimationFrame(()=>{if(!target||!wrap||!shell)return;target.style.top=isCard?'50%':'14px';target.style.transformOrigin=isCard?'center center':'top center';target.style.transform=isCard?'translate(-50%,-50%) scale(1)':'translateX(-50%) scale(1)';const rect=wrap.getBoundingClientRect();const w=Math.max(rect.width,wrap.scrollWidth,680);const h=Math.max(rect.height,wrap.scrollHeight,760);const vw=Math.max(document.documentElement.clientWidth,320);const vh=Math.max(document.documentElement.clientHeight,260);const rawScale=isCard?Math.min((vw-24)/w,(vh-24)/h,1):Math.min((vw-28)/w,1);const scale=Math.max(rawScale,.08);target.style.transform=isCard?'translate(-50%,-50%) scale('+scale+')':'translateX(-50%) scale('+scale+')';if(isCard){shell.style.height='100%';document.body.style.height='100%';}else{const scaledHeight=Math.ceil(h*scale+28);shell.style.height=scaledHeight+'px';document.body.style.height=scaledHeight+'px';if(window.frameElement){window.frameElement.style.height=Math.max(720,scaledHeight)+'px';}}window.frameElement?.classList.add('dds-preview-ready');window.frameElement?.classList.remove('dds-preview-loading');});}window.addEventListener('load',fit);window.addEventListener('resize',fit);document.fonts?.ready?.then(fit);new ResizeObserver(fit).observe(wrap);setTimeout(fit,120);setTimeout(fit,500);setTimeout(fit,1200);window.__ddsFitMusic=fit;fit();})();<\/script></body></html>`;
  }
